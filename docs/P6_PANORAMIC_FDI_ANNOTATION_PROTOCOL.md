...
Status: **READY FOR DATA-RIGHTS GATE — NO PRODUCT WIRING**.
...
- `Panoramic Dental Xray Dataset V3`, DOI `10.17632/73n3kz2k4k.3`: dataset record is CC BY 4.0, but the inspected V3 annotation mirror shows only three polygon-annotated images in `annotations.json`, empty `region_attributes`, and no source FDI labels. It is an eligible image/auxiliary-annotation lead, not a ready FDI corpus.
