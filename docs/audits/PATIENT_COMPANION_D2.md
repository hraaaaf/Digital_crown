...
- Le header patient n'est plus sticky sous le breakpoint `lg`. Cette extension du seuil corrige le recouvrement Companion observé à 768×1024 tout en conservant le sticky desktop à partir de 1024 px.
...
- Le commit produit `0c9f5da34b3cb5517212509f2aae66e8fc7b1607` contient le correctif tablette `lg:sticky`. Les runs PR déclenchés directement par ce commit bot ont été classés `action_required` par GitHub ; ce statut ne constitue pas une validation produit.
