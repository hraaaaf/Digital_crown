...
def _safe_pdf_path(value: Any) -> bool:
    ...
    return bool(
        ...
        and path.parts[:3] == ("backend", "data", "rcp")
...
def _validate_reviewer_contract(errors: list[str]) -> dict[str, bool]:
    ...
    checks = {
        ...
        "review_decision_contract": all(
            token in review_skill
            for token in ("approve", "approve_with_reservations", "request_changes", "blocked")
        ),
    ...
    for index, entry in enumerate(entries):
        ...
        if status == "PENDING_DOWNLOAD":
            if any(
                ...
                for field in ("rcp_sha256", "rcp_checked_at", "local_artifact_path", "unavailability_evidence")
            ):
        ...
        # SNAPSHOT_VERIFIED: require the real repository artifact and verify it independently.
        rcp_url = entry.get("rcp_url")
...
def main() -> int:
    ...
    errors: list[str] = []
    warnings = [
        "This deterministic gate does not constitute scientific or clinical approval.",
        "Independent scientific-reviewer and clinician gates remain applicable where required.",
    ]
    reviewer_checks = _validate_reviewer_contract(errors)
    entry_count, status_counts = _validate_manifest(errors)

    result = {
        "schema_version": "pharmacology-deterministic-scientific-safety.1",
        "gate_type": "DETERMINISTIC_SAFETY_NOT_SCIENTIFIC_APPROVAL",
        "candidate_head": os.environ.get("PRODUCT_HEAD") or os.environ.get("GITHUB_SHA"),
        "status": "PASS" if not errors else "FAIL",
        "independent_scientific_review_required": True,
        "clinical_activation_authorized": False,
        "reviewer_contract": reviewer_checks,
    ...
    summary = [
        "# Pharmacology deterministic scientific safety gate",
        "",
        f"- status: **{result['status']}**",
        f"- candidate HEAD: `{result['candidate_head']}`",
        f"- manifest entries: {entry_count}",
        f"- status counts: `{json.dumps(status_counts, sort_keys=True)}`",
        "- scientific approval: **NO**",
        "- independent scientific review still required: **YES**",
        "",
        "## Errors",
    ]
