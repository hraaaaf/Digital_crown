...
# 2) Roadmap Document Studio globale
p = Path('DOCUMENT_STUDIO_ROADMAP.md')
text = p.read_text(encoding='utf-8')
text = text.replace('CI push post-merge : run `31898590067` — à vérifier avant fermeture documentaire complète.', 'CI push post-merge : run `31898590067` — **SUCCESS**.')
p.write_text(text, encoding='utf-8')

# 3) Audit canonique P1
p = Path('docs/audits/DOCUMENT_STUDIO_P1_ORDONNANCE_AUDIT.md')
text = p.read_text(encoding='utf-8')
text = text.replace('CI push exacte du merge `91a2c2efd781fd736ebdc96e9de4f5e3c73c82c8` : run `31898590067` — à vérifier avant closeout documentaire final.', 'CI push exacte du merge `91a2c2efd781fd736ebdc96e9de4f5e3c73c82c8` : run `31898590067` — **SUCCESS**.')
p.write_text(text, encoding='utf-8')
