...
const output = 'mobile-m4-final-after-artifacts';
...
async function metrics(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      bodyText,
      rawNetworkErrorVisible: /failed to fetch|networkerror|err_connection|load failed/i.test(bodyText),
      friendlyNetworkMessageVisible: /Serveur du cabinet inaccessible\. Vérifiez que le poste cabinet est démarré et accessible sur ce réseau, puis réessayez\./i.test(bodyText),
...
async function openError(mode, viewport) {
  ...
  await page.goto(`http://127.0.0.1:4188/mobile-m4-final-after.html?mode=${mode}`, { waitUntil: 'networkidle' });
...
async function openReadyReturn(viewport) {
  ...
  await page.goto('http://127.0.0.1:4188/mobile-m4-final-after.html?mode=ready', { waitUntil: 'networkidle' });
...
try {
  ...
  const report = {
    ...
    backendOfflineFriendly: offlineMobile.metrics.friendlyNetworkMessageVisible && offlineTablet.metrics.friendlyNetworkMessageVisible,
    ...
    noTouchTargetBelow44: captures.every(item => item.metrics.buttonsBelow44.length === 0),
  ...
  if (report.count !== 6 || !report.unpairedExplicit || !report.revocationExplicit || !report.deletedExplicit || !report.backendOfflineStateVisible || !report.backendOfflineFriendly || report.backendOfflineRawTechnical || !report.returnToAgenda || !report.noHorizontalOverflow || !report.noTouchTargetBelow44 || !report.noUnexpectedRuntimeErrors) {
    throw new Error('M4 final AFTER evidence gate failed.');
  }
} finally {
  await browser.close();
  await server.close();
