...
## BEFORE
Exact product surface: current `OrthoLongitudinalComparePanel` on the F5 branch before any F5 frontend code.
...
The workflow reuses the already-certified F3 capture harness and rewrites the evidence as F5 BEFORE, guaranteeing that the current F3 surface is the comparison baseline.
