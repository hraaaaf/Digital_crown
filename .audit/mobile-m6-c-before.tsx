...
declare global {
  interface Window {
    __M6C_BLANK_SAVE_COUNT__?: number;
...
function AuditApp() {
  ...
  const save = (dataUrl: string) => {
    window.__M6C_BLANK_SAVE_COUNT__ = (window.__M6C_BLANK_SAVE_COUNT__ || 0) + 1;
    ...
    <div className="min-h-[100dvh] bg-background text-text-main font-outfit px-5 py-6" style={{ backgroundColor: 'var(--bg-medical-pearl)' }}>
      ...
      <main className="relative z-10 max-w-md mx-auto space-y-4">
        <div className="px-1">
          ...
          <p className="text-xs text-text-muted mt-1">BEFORE exact produit · BENNANI Sara</p>
        </div>
        <ApptCard
          apt={appointment}
          onStatusChange={() => {}}
