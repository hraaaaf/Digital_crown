"""Seventh batch — SecureTemplateRenderer, data_sanitizer PID/stream/date paths,
...
class TestSecureTemplateRendererSecurityCheck:
    def _renderer(self):
    ...
    def test_safe_html_passes(self):
        r = self._renderer()
        r._security_check("<h1>Bonjour {{ patient }}</h1>")  # should not raise

    def test_import_raises(self):
        r = self._renderer()
        with pytest.raises(ValueError):
            r._security_check("import os; os.system('ls')")

    def test_eval_raises(self):
        r = self._renderer()
        with pytest.raises(ValueError):
            r._security_check("{{ eval('os.system(\"ls\")') }}")

    def test_exec_raises(self):
        r = self._renderer()
        with pytest.raises(ValueError):
            r._security_check("{{ exec('bad code') }}")

    def test_os_dot_raises(self):
        r = self._renderer()
        with pytest.raises(ValueError):
            r._security_check("{{ os.getcwd() }}")
    ...
    def test_config_variable_raises(self):
        r = self._renderer()
        with pytest.raises(ValueError):
            r._security_check("{{ config.SECRET_KEY }}")

    def test_self_variable_raises(self):
        r = self._renderer()
        with pytest.raises(ValueError):
            r._security_check("{{ self.__class__ }}")
...
class TestSecureTemplateRendererRender:
    def _renderer(self):
        from backend.services.template_engine import SecureTemplateRenderer
        return SecureTemplateRenderer()
    ...
    def test_render_simple_template(self):
        r = self._renderer()
        html = "<p>Bonjour {{ patient }}</p>"
        result = r.render(html, {"patient": "Ahmed"})
        assert "Ahmed" in result

    def test_render_allowed_variable(self):
        r = self._renderer()
        html = "<p>{{ patient_nom }} {{ patient_prenom }}</p>"
        result = r.render(html, {"patient_nom": "HASSAN", "patient_prenom": "Ahmed"})
        assert "HASSAN" in result
        assert "Ahmed" in result

    def test_render_filters_unknown_variable(self):
        r = self._renderer()
        # "secret" is not in ALLOWED_VARIABLES — filtered out before rendering
        html = "{% if secret is defined %}SECRET{% else %}SAFE{% endif %}"
        result = r.render(html, {"secret": "password", "patient": "test"})
        assert "SECRET" not in result

    def test_render_date_variable(self):
        r = self._renderer()
        html = "Date: {{ date }}"
        result = r.render(html, {"date": "15/03/2025"})
        assert "15/03/2025" in result

    def test_render_empty_context(self):
        r = self._renderer()
        html = "<p>Aucune variable</p>"
    ...
    def test_render_raises_on_injection(self):
        r = self._renderer()
        with pytest.raises(ValueError):
            r.render("{{ os.getcwd() }}", {})
