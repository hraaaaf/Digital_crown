...
def test_linked_plan_rejects_acte_from_another_patient(client, db, auth_headers, dentiste):
    ...
    response = client.post(
        "/api/installments/",
        json=_payload(patient_a.id, acte_b.id, [1000.0]),
...
def test_linked_plan_must_cover_exact_current_remaining_due(client, db, auth_headers, dentiste):
    ...
    wrong = client.post(
        "/api/installments/",
        json=_payload(patient.id, acte.id, [500.0]),
        headers=auth_headers,
    )
    ...
    exact = client.post(
        "/api/installments/",
        json=_payload(patient.id, acte.id, [375.0, 375.0]),
        headers=auth_headers,
    )
...
def test_installment_payments_keep_both_links_and_reconcile_acte_status(client, db, auth_headers, dentiste):
    ...
    create = client.post(
        "/api/installments/",
        json=_payload(patient.id, acte.id, [400.0, 600.0]),
        headers=auth_headers,
    )
    ...
    first = client.put(
        f"/api/installments/{installments[0]['id']}",
    ...
    payment_one = db.query(models.Payment).filter(
        models.Payment.installment_id == installments[0]["id"]
    ).one()
    assert payment_one.patient_id == patient.id
    assert payment_one.acte_id == acte.id

    second = client.put(
        f"/api/installments/{installments[1]['id']}",
    ...
    payment_two = db.query(models.Payment).filter(
        models.Payment.installment_id == installments[1]["id"]
    ).one()
