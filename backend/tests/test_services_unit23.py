...
class TestUpdateActiveFonts:
    ...
    def test_playfair_legacy_id_maps_to_real_serif_font(self):
        bt = _bt()
        bt.update_active_fonts({'font_fr': 'playfair'})
        # R4 keeps the persisted legacy ID but maps it to a font that is
        # guaranteed by the PDF runtime instead of pretending Playfair is bundled.
        assert bt.premium_font == 'Times-Roman'
        assert bt.premium_bold == 'Times-Bold'
...
class TestScaleElements:
    ...
    def test_scale_spacer_reduces_height(self):
        ...
        assert result[0].height <= 20 * 0.5 + 1
