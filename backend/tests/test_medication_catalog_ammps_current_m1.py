...
def test_current_snapshot_is_isolated_from_historical_search_api():
    rows = medication_dict.search("AMOXICILLINE LLORENTE", limit=20)
...
def test_get_regulatory_presentation_resolves_exact_package():
    row = next(
        ...
        if item.get("source", {}).get("id") == CURRENT_SOURCE_ID
        and item.get("presentation") == "BOITE DE 24"
        and item.get("dosage") == "1"
    )
