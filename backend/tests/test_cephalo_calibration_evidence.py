...
def _initial():
    ...
    return build_cephalo_runtime_evidence_payload(
        patient_id=7, image_record_id="radio.jpg", result=result, landmarks=raw,
        inference_mode="SOTA_ONNX_38", case_id=CASE_ID, recorded_at=NOW,
    )


def _calibrated(previous=None, raw=None):
    previous = previous or _initial(); raw = raw or _raw()
    result = CephaloEngine(mm_per_pixel=0.2).calculate_metrics(_points(raw))
    return rebuild_evidence_after_manual_calibration(
        previous_payload=previous, patient_id=7, image_record_id="radio.jpg", result=result,
        runtime_landmarks=raw, p1={"x": 0.0, "y": 0.0}, p2={"x": 0.0, "y": 50.0},
        distance_mm=10.0, clinician_id="99", calibrated_at=NOW,
    )


def test_manual_calibration_creates_audited_revision_unlocks_linear_and_preserves_angular():
    previous = _initial(); payload = _calibrated(previous)
...
def test_calibration_rejects_runtime_landmarks_different_from_persisted_evidence():
    raw = _raw(); raw[4] = {**raw[4], "x": raw[4]["x"] + 5.0}
...
def test_calibration_rejects_wrong_cephalogram_identity():
    previous = _initial(); result = CephaloEngine(mm_per_pixel=0.2).calculate_metrics(_points())
    with pytest.raises(CephaloRuntimeEvidenceError, match="source record mismatch"):
        rebuild_evidence_after_manual_calibration(
            previous_payload=previous, patient_id=7, image_record_id="other-radio.jpg", result=result,
            runtime_landmarks=_raw(), p1={"x": 0.0, "y": 0.0}, p2={"x": 0.0, "y": 50.0},
            distance_mm=10.0, clinician_id="99", calibrated_at=NOW,
        )
