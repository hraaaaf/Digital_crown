...
def _supplier(db, owner, key, active=True):
    item = models.PartnerSupplier(
        employer_id=owner.get_employer_id(),
        supplier_key=key,
        name=f"Supplier {key}",
        sync_mode="api",
        api_base_url="https://supplier.example.com/api",
        is_active=active,
    )
    db.add(item)
    db.commit()
...
def _order(db, owner, suffix, revenue=20.0):
    item = models.PartnerOrder(
        employer_id=owner.get_employer_id(),
        order_number=f"GOV-{owner.id}-{suffix}",
        partner_id="1",
...
def test_sensitive_governance_requires_confirmation_and_writes_audit(db):
    ...
    result = update_governance(
        ...
        SupplierGovernanceIn(
            ...
            expiresAt=effective + timedelta(days=365),
        ),
        db=db,
        admin=admin,
    )
...
def test_partial_date_patch_validates_final_persisted_agreement(db):
    admin = _user(db, "gov-date-admin@test.ma")
    owner = _user(db, "gov-date-owner@test.ma")
    supplier = _supplier(db, owner, "dates")
    effective = datetime.utcnow() + timedelta(days=10)
    ...
    with pytest.raises(HTTPException) as invalid:
        update_governance(
            supplier.id,
            SupplierGovernanceIn(confirm=True, expiresAt=effective - timedelta(days=1)),
            db=db,
            admin=admin,
        )
    assert invalid.value.status_code == 422
    db.refresh(agreement)
    assert agreement.expires_at == effective + timedelta(days=30)
    assert db.query(MarketplaceGovernanceEvent).count() == 0
...
def test_expired_active_agreement_is_reported_effectively_expired(db):
    ...
    payload = _agreement_payload(agreement)
    assert payload["status"] == "EXPIRED"
    assert payload["storedStatus"] == "ACTIVE"
