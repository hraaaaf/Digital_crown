...
def _srpose_points():
    coords = {name: (float(100 + index * 2), float(120 + index * 3)) for index, name in SOTA_LANDMARKS_MAPPING.items()}
    coords.update({key: value for key, value in _engine_points().items() if key in coords})
    ...
    return [{"id": name, "x": x, "y": y} for name, (x, y) in coords.items()]
...
class _Repo:
    ...
    def update(self, _analysis_id, _landmarks, results, mm_per_pixel=None):
        self.persisted = results
        return SimpleNamespace(id=42, is_calibrated=False, mm_per_pixel=mm_per_pixel, landmarks_data=_landmarks)
...
def test_new_analysis_persists_evidence_without_changing_public_result(monkeypatch):
    ...
    repo = _Repo()
    service.repo = repo
    monkeypatch.setattr("backend.services.cephalo_service.vision_engine.predict_landmarks", lambda _path: {"landmarks": _srpose_points(), "mode_inference": "SOTA_ONNX_38", "warning": None, "processing_time_ms": 1.0})
    monkeypatch.setattr("backend.services.calibration_service.calibration_service.detect_mm_per_pixel", lambda _path: None)
    monkeypatch.setattr("backend.services.cephalo_service.cephalo_engine.calculate_metrics", lambda *_args, **_kwargs: _result())
    ...
    craniom_angular = [item for item in graph["measurements"] if item["analysis_id"] == "CRANIOM" and not item["requires_calibration"]]
...
def test_refine_replaces_current_manual_revision_but_preserves_original_auto(monkeypatch):
    ...
    existing = SimpleNamespace(patient_id=7, patient=None, image_original_path="api/static/radio.jpg", angles_data=initial_repo.persisted, mm_per_pixel=None, is_calibrated=False, calibration_data=None, created_at=NOW)
    service = CephaloService(MagicMock())
    repo = _Repo(existing=existing)
    service.repo = repo
    monkeypatch.setattr("backend.services.cephalo_service.bilan_ortho_engine.generate_bilan", lambda *_args, **_kwargs: {})
    manual = [{"id": key, "x": x + 1.0, "y": y} for key, (x, y) in _engine_points().items()]
