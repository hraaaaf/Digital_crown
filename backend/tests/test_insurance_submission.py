...
from backend.services.insurance_submission import (
    archive_validated_insurance_pdf,
...
def test_ready_for_review_requires_exact_mapping_and_no_missing_fields():
    ...
    assert draft.status == InsuranceDraftStatus.READY_FOR_REVIEW
...
def test_honoraires_adapter_reads_teeth_from_snapshot_and_aligns_acte_ids():
...
def test_historical_line_count_mismatch_fails_closed():
    with pytest.raises(ValueError, match="line count mismatch"):
        build_draft_from_honoraires_snapshot(
            ...
            organization=InsuranceOrganization.CNSS,
            honoraires_document_id=42,
            payments=[{"acte": "Detartrage", "montant": 500}],
            active_acte_ids=[70, 71],
            template_version="CNSS-610-1-04",
        )
...
def test_archive_rejects_unvalidated_draft_before_touching_db():
...
def test_archive_rejects_non_pdf_before_touching_db():
    validated = _draft(
        status=InsuranceDraftStatus.VALIDATED,
        lines=[_exact_line()],
        validated_by_practitioner_id=3,
