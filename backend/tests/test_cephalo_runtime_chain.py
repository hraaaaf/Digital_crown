...
def test_get_fails_closed_when_historical_landmark_makes_current_authority_ambiguous():
    ...
    duplicate["model_id"] = duplicate["model_sha256"] = duplicate["pipeline_version"] = None
...
def test_creation_edit_calibration_recalculation_and_get_keep_one_active_chain():
    ...
    next(item for item in edited if item["id"] == "A")["x"] += 2.0
    ...
    next(item for item in recalculated if item["id"] == "B")["x"] += 1.0
    ...
    angular = [measurement for measurement in revision4["measurements"] if not measurement["requires_calibration"]]
    assert {measurement["method_id"] for measurement in angular} == {"CRANIOM_U1_FRANKFORT_DEG_V1","CRANIOM_L1_DOWNS_DEG_V1","CRANIOM_INTERINCISAL_DEG_V1"}
