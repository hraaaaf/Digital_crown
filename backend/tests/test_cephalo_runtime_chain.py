...
def _assert_mcnamara(graph, *, calibrated: bool):
    mcnamara = [
        measurement for measurement in graph["measurements"]
    ...
    if calibrated:
        ...
        assert all(measurement["value"] is None for measurement in mcnamara)
...
def test_creation_edit_calibration_recalculation_and_get_keep_one_active_chain():
    ...
    _assert_mcnamara(initial_graph, calibrated=False)
    ...
    _assert_mcnamara(revision2, calibrated=False)
    ...
    _assert_mcnamara(revision3, calibrated=True)
    ...
    _assert_mcnamara(revision4, calibrated=True)
    ...
    assert all(
        ...
        and measurement["requires_calibration"]
    )

    craniom_angular = [
        measurement for measurement in revision4["measurements"]
        if measurement["analysis_id"] == "CRANIOM"
        and not measurement["requires_calibration"]
    ]
    assert {measurement["method_id"] for measurement in craniom_angular} == {
    ...
    steiner = [
        measurement for measurement in revision4["measurements"]
        if measurement["analysis_id"] == "STEINER"
    ]
    assert {measurement["method_id"] for measurement in steiner} == {
        "STEINER_SNA_DEG_V1",
        "STEINER_SNB_DEG_V1",
        "STEINER_ANB_DEG_V1",
