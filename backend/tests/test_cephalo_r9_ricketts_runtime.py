...
def _initial(raw=None):
    ...
    return build_cephalo_runtime_evidence_payload(
        ...
        landmarks=raw, inference_mode="SOTA_ONNX_38", case_id=CASE_ID,
        recorded_at=NOW,
    )
...
def _ricketts(graph, *, calibrated: bool):
    ...
    facial = by_method["RICKETTS_FACIAL_DEPTH_DEG_V1"]
    assert facial["availability_status"] == "AVAILABLE"
    assert facial["requires_calibration"] is False
    assert facial["calibration_ref"] is None
    assert facial["value"] is not None
...
def test_ricketts_creation_edit_manual_calibration_edit_and_read_path():
    ...
    next(item for item in edited if item["id"] == "Pog")["x"] += 8.0
    ...
    revision2 = rebuild_evidence_after_landmark_edit(
        ...
        result=edit_result, runtime_landmarks=edited, clinician_id="clinician-r9",
        validated_at=LATER,
    )
    edited_ricketts = _ricketts(revision2, calibrated=False)
    assert edited_ricketts["RICKETTS_FACIAL_DEPTH_DEG_V1"]["value"] != pytest.approx(
        initial_ricketts["RICKETTS_FACIAL_DEPTH_DEG_V1"]["value"]
    )
    ...
    next(item for item in recalculated if item["id"] == "Pog")["x"] += 3.0
    ...
    revision4 = rebuild_evidence_after_landmark_edit(
        ...
        result=recalc_result, runtime_landmarks=recalculated, clinician_id="clinician-r9",
        validated_at=LATEST,
    )
    recalculated_ricketts = _ricketts(revision4, calibrated=True)
    assert recalculated_ricketts["RICKETTS_FACIAL_DEPTH_DEG_V1"]["value"] != pytest.approx(
        calibrated_ricketts["RICKETTS_FACIAL_DEPTH_DEG_V1"]["value"]
    )
...
def test_ricketts_auto_calibration_unlocks_e_line_and_preserves_facial_depth():
    raw = _raw()
    initial = _initial(raw)
    initial_ricketts = _ricketts(initial, calibrated=False)
