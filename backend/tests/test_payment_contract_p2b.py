...
def test_payment_amount_must_be_strictly_positive(client, db, auth_headers, dentiste):
    ...
    for amount in (0, -10):
        response = client.post(
            ...
            json={"patient_id": patient.id, "amount": amount, "payment_method": "ESPECES"},
...
def test_payment_method_must_be_known(client, db, auth_headers, dentiste):
    ...
    response = client.post(
        "/api/accounting/payments",
        json={"patient_id": patient.id, "amount": 250, "payment_method": "INVENTE"},
        headers=auth_headers,
    )
    assert response.status_code == 422, response.text
    assert db.query(models.Payment).filter(models.Payment.patient_id == patient.id).count() == 0
...
def test_valid_explicit_payment_persists_exact_amount_and_method(client, db, auth_headers, dentiste):
    ...
    assert response.json()["amount"] == 375.5
    assert response.json()["payment_method"] == "CHEQUE"
...
def test_known_ui_payment_method_alias_is_normalized(client, db, auth_headers, dentiste):
    ...
    response = client.post(
        "/api/accounting/payments",
        json={"patient_id": patient.id, "amount": 420, "payment_method": "TPE"},
        headers=auth_headers,
    )
    assert response.status_code == 200, response.text
