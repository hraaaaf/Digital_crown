...
from backend.services.cephalo_auto_calibration_gate import AutoCalibrationDecision, AutoCalibrationState, ValidatedFiducialProfile, evaluate_auto_calibration
from backend.services.cephalo_auto_calibration_transition import AutoCalibrationTransitionError, auto_calibration_data_from_decision, rebuild_evidence_after_auto_calibration
...
def _initial(raw=None):
    raw = raw or _raw(); result = CephaloEngine(mm_per_pixel=None).calculate_metrics(_points(raw))
...
def _decision():
    ...
    profile = ValidatedFiducialProfile(profile_id="TEST_RULER", version="1", known_tick_spacing_mm=5.0, min_ticks=5, max_spacing_deviation_ratio=0.05, validation_reference="test-fixture://validated-ruler-profile-v1")
...
def _auto(previous=None, raw=None, *, at=NOW, image_record_id="radio.jpg"):
    previous = previous or _initial(); raw = raw or _raw(); result = CephaloEngine(mm_per_pixel=0.2).calculate_metrics(_points(raw))
    return rebuild_evidence_after_auto_calibration(previous_payload=previous, patient_id=7, image_record_id=image_record_id, result=result, runtime_landmarks=raw, decision=_decision(), calibrated_at=at)


def test_auto_calibration_creates_revision_and_unlocks_measurements_without_clinician_confirmation():
    previous = _initial(); payload = _auto(previous)
    ...
    assert {item["method_id"] for item in steiner} == {"STEINER_SNA_DEG_V1", "STEINER_SNB_DEG_V1", "STEINER_ANB_DEG_V1", "STEINER_SN_MP_DEG_V1", "STEINER_U1_NA_DEG_V1", "STEINER_L1_NB_DEG_V1"}
    ...
    assert {item["method_id"] for item in mcnamara} == {"MCNAMARA_CO_A_MM_V1", "MCNAMARA_CO_GN_MM_V1", "MCNAMARA_ANS_ME_MM_V1"}
...
def test_auto_calibration_preserves_corrected_landmark_and_current_refs_exactly():
    raw = _raw(); edited = [dict(item) for item in raw]
    target = next(item for item in edited if item["id"] == "A"); target["x"] += 4.0
    edit_result = CephaloEngine(mm_per_pixel=None).calculate_metrics(_points(edited))
    corrected = rebuild_evidence_after_landmark_edit(previous_payload=_initial(raw), patient_id=7, image_record_id="radio.jpg", result=edit_result, runtime_landmarks=edited, clinician_id="99", validated_at=NOW)
...
def test_auto_calibration_rejects_runtime_landmarks_different_from_current_evidence():
    previous = _initial(); changed = _raw(); changed[4] = {**changed[4], "x": changed[4]["x"] + 1.0}
