...
def _owner(db: Session, employer_id: int):
    ...
    return {
        ...
        "ownerName": user.nom_complet if user else None,
    }
...
def overview(
    ...
    return {
        ...
        "activeSuppliersCount": sum(bool(x.is_active) for x in suppliers),
...
def update_governance(
    ...
    current_effective = agreement.effective_at if agreement else None
    current_expires = agreement.expires_at if agreement else None
    final_effective = payload.effectiveAt if payload.effectiveAt is not None else current_effective
    final_expires = payload.expiresAt if payload.expiresAt is not None else current_expires
    if final_effective and final_expires and final_expires <= final_effective:
    ...
    agreement_mutation = any(
        x is not None
        for x in (
            payload.agreementStatus,
            payload.agreementReference,
            payload.effectiveAt,
            payload.expiresAt,
            payload.notes,
        )
    )
    if agreement_mutation:
        if agreement is None:
            agreement = PartnerSupplierAgreement(employer_id=supplier.employer_id, supplier_id=supplier.id)
            db.add(agreement)
        if payload.agreementStatus is not None:
            agreement.status = payload.agreementStatus
        if payload.agreementReference is not None:
            agreement.reference = payload.agreementReference.strip() or None
        if payload.effectiveAt is not None:
            agreement.effective_at = payload.effectiveAt
        if payload.expiresAt is not None:
            agreement.expires_at = payload.expiresAt
        if payload.notes is not None:
            agreement.notes = payload.notes.strip() or None
