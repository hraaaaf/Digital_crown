...
# A4 must register its table and nullable Appointment.resource_id before startup
# create_all() in isolated development/test runtimes. Cabinet/production remain
# Alembic-authoritative.
from backend.models_agenda_a4 import install_agenda_resource_model
...
patients.router.routes = [
    ...
    if not (getattr(route, "path", None) == "/{patient_id}/master-plan" and ({"GET", "PUT"} & (getattr(route, "methods", set()) or set())))
]
patients.router.routes = [
    route for route in patients.router.routes
    if not (getattr(route, "path", None) == "/{patient_id}/journey" and "GET" in (getattr(route, "methods", set()) or set()))
]
patients.router.routes = [
    route for route in patients.router.routes
    if not (getattr(route, "path", None) == "/{patient_id}/financial-snapshot" and "GET" in (getattr(route, "methods", set()) or set()))
]
