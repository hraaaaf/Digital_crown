"""Contextual mobile resource bridge: patient and panoramic resources."""
...
def _resource_permission(resource_type: str) -> str:
    return str(_RESOURCE_SPECS[_resource_type(resource_type)]["permission"])
...
def _resource_bridge_target_users(db: Session, current_user: models.User, resource_type: str) -> list[models.User]:
    permission = _resource_permission(resource_type)
...
def _context_state(db: Session, mobile_user: models.User, context) -> tuple[str, str | None]:
    ...
    if not has_permission(mobile_user, permission):
        return "unavailable", f"La permission {permission} n'est plus disponible pour cette session."

    try:
        _resource_entity(db, mobile_user, resource_type, int(context["resource_id"]))
    except HTTPException as exc:
        if exc.status_code in (403, 404):
            return "unavailable", str(exc.detail)
        raise
    return "ready", None
...
def _validated_mobile_context(
    ...
    if state != "ready":
        permission = _resource_permission(str(context["resource_type"])) if str(context["resource_type"]) in _RESOURCE_SPECS else "patients"
        raise HTTPException(
            status_code=403 if not has_permission(mobile_user, permission) else 404,
            detail=reason or "Ressource mobile indisponible.",
        )
...
def get_resource_bridge_options(
    ...
    resource_type = _resource_type(resource_type)
    _resource_entity(db, current_user, resource_type, resource_id)
    targets = _resource_bridge_target_users(db, current_user, resource_type)
    return {
...
def create_resource_bridge_pairing(
    ...
    target = _resolve_resource_bridge_target(db, current_user, body.target_user_id, resource_type)
...
def get_resource_context_media(
    ...
    mobile_user, context = _validated_mobile_context(db, authorization, body.context_key)
    if str(context["resource_type"]) != "panoramic":
        raise HTTPException(status_code=422, detail="Ce contexte mobile ne contient pas de média panoramique.")
