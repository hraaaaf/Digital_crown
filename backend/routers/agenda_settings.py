...
def get_cabinet_settings(db: Session = Depends(get_db)):
...
def update_cabinet_settings(settings_update: CabinetSettingsUpdate, db: Session = Depends(get_db)):
    settings = db.query(models.CabinetSettings).first()
    if not settings:
        settings = models.CabinetSettings()
        db.add(settings)
...
def create_exception(exc_in: AgendaExceptionCreate, db: Session = Depends(get_db)):
