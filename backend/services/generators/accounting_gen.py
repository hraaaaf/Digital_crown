...
class AccountingGenerator:
    ...
    def _amount_to_words(self, amount):
        ...
        def _to_words_int(n):
            ...
            if n < 1000:
                ...
                return prefix + "Cent" + ("s" if r == 0 and q > 1 else "") + (" " + _to_words_int(r).lower() if r > 0 else "")
            return str(int(n))
    ...
    def _create_header(self, patient, data, p_color):
        ...
        patient_style = ParagraphStyle(name='PatientInfo', parent=self.styles['Normal'], fontName='Helvetica-Bold', fontSize=10, textColor=p_color, leading=12)
        style_right = ParagraphStyle('DocDate', parent=self.styles['Normal'], alignment=TA_RIGHT, textColor=p_color)
        header_content = [[Paragraph(f"Nom : {patient.nom.upper()} {patient.prenom.capitalize()}<br/>Âge : {age} ans", patient_style), Paragraph(f"Le : ....{current_date}....", style_right)]]
    ...
    def generate_note(self, patient, data, facture_number=None, db=None, user_id=None, **kwargs):
        ...
        title_style = ParagraphStyle(name='TitleA5', parent=self.styles['Normal'], fontName='Helvetica-Bold', fontSize=16, textColor=p_color, alignment=TA_CENTER, spaceAfter=12)
        elements = [Spacer(1, 0.5*cm), Paragraph(f"NOTE D'HONORAIRES N° {facture_number}" if facture_number else "NOTE D'HONORAIRES", title_style), Spacer(1, 1.0*cm), self._create_header(patient, data, p_color), Spacer(1, 1.5*cm)]
        
        header_style = ParagraphStyle(name='TableHeader', parent=self.styles['Normal'], fontName='Helvetica-Bold', fontSize=8, textColor=colors.white, alignment=TA_CENTER)
        ...
        text_style = ParagraphStyle(name='TableText', parent=self.styles['Normal'], fontName='Helvetica', fontSize=9, textColor=p_color, alignment=TA_CENTER)
        acte_style = ParagraphStyle(name='ActeText', parent=self.styles['Normal'], fontName='Helvetica', fontSize=9, textColor=p_color, alignment=TA_LEFT, leading=11)
        ...
        table_data.append(["", "", Paragraph("<b>TOTAL</b>", text_style), Paragraph(f"<b>{total:.2f}</b>", text_style)])
        t = Table(table_data, colWidths=[4.3*cm, 1.5*cm, 3.0*cm, 3.0*cm])
        t.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), p_color), ('ALIGN', (0,0), (-1,-1), 'CENTER'), ('VALIGN', (0,0), (-1,-1), 'TOP'), ('GRID', (0,0), (-1,-2), 0.3, p_color), ('TEXTCOLOR', (0,1), (-1,-1), p_color), ('WORDWRAP', (0,0), (-1,-1), True)]))
        elements.append(t)
        
        total_words = self._amount_to_words(total)
        cloture = f"Arrêtée la présente note à la somme de {total_words} TTC ({total:.2f} MAD)."
        user_obj = None
        if db and user_id:
    ...
    def generate_devis(self, patient, data, document_number=None, db=None, user_id=None, **kwargs):
        ...
        title_style = ParagraphStyle(name='TitleA5', parent=self.styles['Normal'], fontName='Helvetica-Bold', fontSize=16, textColor=p_color, alignment=TA_CENTER, spaceAfter=12)
        elements = [Spacer(1, 0.5*cm), Paragraph(f"DEVIS N° {document_number}" if document_number else "DEVIS DENTAIRE", title_style), Spacer(1, 1.0*cm), self._create_header(patient, data, p_color), Spacer(1, 1.5*cm)]
        ...
        total_words = self._amount_to_words(total)
        cloture = f"Arrêté le présent devis à la somme de {total_words} TTC ({total:.2f} MAD)."
        user_obj = None
        if db and user_id:
            from backend.models import User
            user_obj = db.query(User).filter(User.id == user_id).first()
