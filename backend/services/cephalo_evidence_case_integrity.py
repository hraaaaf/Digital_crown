...
def validate_case_evidence_graph(
    ...
    *,
    ...
    dependency chain. This wrapper adds the patient/case and clinician-audit
    invariants needed before the graph can be used as a clinical source of truth.
    """
    ...
    sources_by_id = {source.evidence_id: source for source in graph.sources}
    ...
    for measurement in graph.measurements:
        if measurement.calibration_ref is None:
            continue
        calibration = sources_by_id[measurement.calibration_ref]
        if calibration.kind != "calibration":
            raise EvidenceGraphValidationError(
                f"Measurement {measurement.measurement_id} calibration_ref must target "
                f"SourceEvidence kind='calibration', got {calibration.kind!r}"
            )
