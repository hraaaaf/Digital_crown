...
def _required_positive_number(provenance: Mapping[str, Any], key: str) -> float:
    ...
    if not math.isfinite(number) or number <= 0:
        raise AutoCalibrationEvidenceError(f"{key} must be finite and positive")
    return number
...
def source_evidence_from_auto_decision(
    *,
    ...
    try:
        candidate_tick_count = int(provenance.get("candidate_tick_count"))
    except (TypeError, ValueError) as exc:
        raise AutoCalibrationEvidenceError("candidate_tick_count must be an integer") from exc
    if candidate_tick_count < 2:
        raise AutoCalibrationEvidenceError("candidate_tick_count must be at least 2")
    ...
    return SourceEvidence(
        evidence_id="src-calibration-auto-verified",
        patient_id=patient_id,
        kind="calibration",
        source_record_id=image_record_id,
