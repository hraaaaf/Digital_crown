...
# DOI metadata is independently indexed by PubMed. The paper specifies the
# mandibular-plane indicator as true Frankfort to Sub.Go.-M. and explicitly
# describes constructing true Frankfort from true porion/orbitale rather than
# the ear-rod proxy. We therefore model those source-specific points explicitly
# instead of relabelling generic SRPose38 Po/Or/Go/Me as source-exact.
...
# Tweed remains intentionally blocked. Tweed's primary 1946 FMA paper and the
# 1954 FMIA paper require a source-specific Frankfort reference and mandibular
# lower-border construction. Existing Go-Me / Go-Gn convenience lines are not
# promoted to historical source-exact geometry without a locked tracing contract.
...
def materialize_ricketts_1981_fma_construction(
    ...
    *,
    ...
    """Materialize Ricketts 1981 FMA geometry only when source points are explicit.

    Four source-specific manual landmarks are required. This deliberately costs
    more tracing effort than reusing SRPose38 Po/Or/Go/Me, because the 1981
    construction distinguishes true Frankfort and Subgonion from convenient
    generic runtime proxies.
    """
