...
def assert_insurance_ngap_source(*, source_store_root: Path, draft: InsuranceSubmissionDraft) -> None:
    ...
    loaded = load_stored_insurance_source(root=source_store_root, namespace="ngap", version=version, sha256=source_hash)
    manifest = loaded.manifest
...
def validate_insurance_draft_by_practitioner(
    ...
    *,
    ...
    practitioner = db.query(models.User).filter(models.User.id == int(practitioner_id), models.User.is_active.is_(True)).first()
    ...
    resolved = apply_ngap_reference_to_draft(db, draft=draft, reference_version=reference_version)
