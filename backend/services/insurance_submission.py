...
def build_draft_from_honoraires_snapshot(*, patient_id: int, organization: InsuranceOrganization,
    ...
    for index, item in enumerate(item_list):
        ...
        lines.append(InsuranceSubmissionLine(
            ...
            service_date=_service_date(item, fallback), label=label, teeth=_normalize_teeth(item),
            amount_mad=amount, mapping_status=InsuranceMappingStatus.NOT_EVALUATED,
        ))
...
def archive_validated_insurance_pdf(db: Session, *, draft: InsuranceSubmissionDraft, pdf_content: bytes,
    ...
    return ArchiveService(db).archive_document(
        patient_id=draft.patient_id, file_content=pdf_content, filename=filename,
