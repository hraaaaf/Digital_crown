...
app = FastAPI(title="Digital Crown API - SANINOVA Edition")
...
@app.put("/cabinet/me", response_model=schemas.CabinetConfigOut)
def update_cabinet_info(settings: CabinetUpdate, db: Session = Depends(database.get_db)):
    """Met à jour les informations globales du cabinet."""
...
@app.get("/cabinet/me", response_model=schemas.CabinetConfigOut)
...
def generate_patient_pdf(patient_id: int, req: schemas.CephaloPDFRequest, db: Session = Depends(database.get_db)):
    ...
    try:
        ...
        pdf_path = doc_factory.create_bilan_report(patient, analysis_data)
    except Exception as e:
        import traceback
...
def generate_document(req: schemas.DocumentRequest, archive: bool = False, db: Session = Depends(database.get_db)):
    ...
    try:
        ...
        if archive:
            ...
            patient_filepath = os.path.join(patient_docs_dir, final_filename)
            
            source_path = os.path.join(BASE_DIR, pdf_path) if not os.path.isabs(pdf_path) else pdf_path
            if os.path.exists(source_path):
                shutil.move(source_path, patient_filepath)
                pdf_path = f"static/patients/{p_folder}/Documents/{final_filename}"
                
                if req.type in ["note", "honoraires"]:
                    archive_type_folder = "NOTE"
                    archive_dir = os.path.join(STATIC_DIR, "archive", archive_type_folder, today.strftime('%Y'), MONTHS_FR[today.month])
                    os.makedirs(archive_dir, exist_ok=True)
                    shutil.copy2(patient_filepath, os.path.join(archive_dir, final_filename))

    except Exception as e:
        logger.error(f"Erreur Archivage : {e}")
        raise HTTPException(status_code=500, detail=str(e))

    return {"status": "success", "pdf_url": pdf_path.replace("\\", "/")}
# ==============================================================================
...
def move_to_trash(
    ...
    try:
        doc = archive_service.move_to_trash(document_id)
...
def restore_from_trash(
    ...
    try:
        doc = archive_service.restore_from_trash(document_id)
