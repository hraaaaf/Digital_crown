...
### Dernières demandes
- essaie de comprendre pourquoi j'ai le D de MAD sur la ligne total général sur une seule ligne tout seul ! alors que je voulais pas de retour en ligne blob:http:
- <task-notification> <task-id>a09fe2f1f3ceb0e90</task-id> <tool-use-id>toolu_01JtAGWjV7i8bn6Bfdzo4Jtn</tool-use-id> <output-file>C:\Users\lenovo\AppData\Local\Te
- normalement le meme concept doit s'appliquer sur les autres documents générés ! normalement ils subissent la meme logique il faut rien faire hardcodé ! il faut 
...
- que le montant ou tout le document généré s'actualise de façon pas fixe ? !
<!-- STATE:AUTO:END -->
