...
export const getCabinetHealthDisplayState = (
  ...
  if (state.status === 'unverified') {
    return { label: 'Système non vérifié', dotClassName: 'bg-amber-500', isLoading: false };
  }
  if (state.data.overall_severity === 'ok') {
    return { label: 'Système opérationnel', dotClassName: 'bg-emerald-500', isLoading: false };
  }
  if (state.data.overall_severity === 'warning') {
    return { label: 'Vigilance requise', dotClassName: 'bg-amber-500', isLoading: false };
  }
  return { label: 'Problème critique', dotClassName: 'bg-red-500', isLoading: false };
};
...
export const useCabinetHealth = ({
  ...
  const [state, setState] = useState<CabinetHealthState>({ status: 'loading', data: null });
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        if (!cancelled) setState({ status: 'unverified', data: null });
      }
    };
