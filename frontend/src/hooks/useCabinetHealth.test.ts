...
const healthy: CabinetHealth = {
  ...
  backup_local: { status: 'ok', overall_status: 'SUCCESS', age_hours: 4, run_id: 'run-d2' },
  offsite: { status: 'NOT_CONFIGURED', offsite_status: null, db_copied: null, media_copied: null },
  overall_severity: 'ok',
};
...
describe('useCabinetHealth — vérité système D2', () => {
  ...
  it('n’affiche opérationnel qu’après une réponse de santé réelle', async () => {
    const fetcher = vi.fn().mockResolvedValue(healthy);
    const { result } = renderHook(() => useCabinetHealth({ enabled: true, authLoading: false, fetcher }));
  ...
  it('passe en non vérifié sur erreur réseau puis récupère au polling suivant', async () => {
    vi.useFakeTimers();
    const fetcher = vi.fn<() => Promise<CabinetHealth>>()
      .mockRejectedValueOnce(new Error('network unavailable'))
      .mockResolvedValueOnce(healthy);
    const { result } = renderHook(() => useCabinetHealth({ enabled: true, authLoading: false, pollMs: 1_000, fetcher }));
  ...
  it('mappe explicitement vigilance et critique', () => {
    expect(getCabinetHealthDisplayState({ status: 'ready', data: { ...healthy, overall_severity: 'warning' } }).label).toBe('Vigilance requise');
    expect(getCabinetHealthDisplayState({ status: 'ready', data: { ...healthy, overall_severity: 'critical' } }).label).toBe('Problème critique');
  });
});
