...
export const AppointmentMobileBridge = ({ appointmentId, appointmentLabel }: { appointmentId: number; appointmentLabel: string }) => {
  ...
  const modal = open && typeof document !== 'undefined' ? createPortal(
    <div className="fixed inset-0 z-[750] bg-slate-950/45 backdrop-blur-sm p-3 sm:p-6 flex items-center justify-center" role="dialog" aria-modal="true" aria-label="Rendez-vous sur mobile" onClick={(event) => event.stopPropagation()}>
      <section data-m4d-bridge className="w-full max-w-xl max-h-[92dvh] overflow-y-auto rounded-[2rem] border border-border-main bg-card-bg shadow-2xl p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4 mb-5">
          ...
          <button data-m4d-touch type="button" onClick={() => setOpen(false)} aria-label="Fermer l’accès mobile rendez-vous" className="min-w-11 min-h-11 rounded-xl border border-border-main inline-flex items-center justify-center text-text-muted hover:text-text-main"><X size={18}/></button>
        </div>
