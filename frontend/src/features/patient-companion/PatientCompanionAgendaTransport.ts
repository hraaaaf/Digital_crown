...
const relayHeaders = (capability: string) => ({
  Authorization: `RelayCap ${capability}`,
});
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    for (const item of items) {
      ...
      await fetch(
        ...
        {
          method: 'DELETE',
          headers: relayHeaders(relay.patientReadCapability),
        },
      ).catch(() => undefined);
      return result;
    }
...
export async function sendAgendaCommand(
  ...
  if (binding.relay) {
    return sendViaRelay(pairing, blob, operation, idempotencyKey, messageId);
  }
  return sendDirect(pairing, blob, operation, idempotencyKey, messageId);
}
