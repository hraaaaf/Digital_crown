    ...
    <div className={cn(
      ...
      <motion.button
        ...
        {...(!isEmbedded ? {
          ...
          dragElastic: 0.1,
          whileDrag: { scale: 1.1, cursor: 'grabbing', boxShadow: '0 0 30px rgba(var(--primary-rgb), 0.4)' }
        } : {})}
      ...
      >
        {isExpanded ? (
          ...
          <div className="relative z-10">
            {insights.length > 0 && (
              <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-primary text-white text-[8px] font-black flex items-center justify-center rounded-full border-2 border-white dark:border-slate-800 animate-bounce">
                {insights.length}
              </span>
            )}
