...
export const StudioTabs: React.FC<StudioTabsProps> = ({ activeTab, onTabChange, 'data-tour': dataTour }) => (
  ...
  >
    ...
    <TabButton active={activeTab === 'honoraires'} onClick={() => onTabChange('honoraires')} icon={<Receipt size={16} />} label="Note Honoraires" tourId="tab-honoraires" />
    <TabButton active={activeTab === 'echeancier'} onClick={() => onTabChange('echeancier')} icon={<Calculator size={16} />} label="Suivi Paiement" tourId="tab-suivi" />
    <TabButton active={activeTab === 'libre'} onClick={() => onTabChange('libre')} icon={<Type size={16} />} label="Document Libre" tourId="tab-libre" />
    <TabButton active={activeTab === 'plan'} onClick={() => onTabChange('plan')} icon={<Brain size={16} />} label="Compagnon Diagnostique" tourId="tab-strategie" />
  </div>
);
