...
describe('Ordonnance Fidelity V3.1 prescription composer', () => {
  it('renders the four structured controls and generated summary', () => {
    ...
    expect(drugRow).toContain('data-composer-summary');
  ...
  it('keeps all structured selectors touch-safe', () => {
    const composerStart = drugRow.indexOf('data-ordonnance-prescription-composer');
    const composerEnd = drugRow.indexOf('{nationalMsg', composerStart);
    const composerSource = drugRow.slice(composerStart, composerEnd);
    expect(composerSource.match(/min-h-11/g)?.length ?? 0).toBeGreaterThanOrEqual(5);
  });
