  ...
  return (
    ...
    >
      <div className="grid grid-cols-12 gap-3 items-center">
        <div className="col-span-2 lg:col-span-1 flex flex-col items-center justify-center self-stretch border-r border-slate-100/50 pr-2">
        ...
        <div className="col-span-2 lg:col-span-1 flex items-center lg:flex-col gap-1 p-1 bg-slate-50/50 rounded-2xl border border-slate-100/50 self-stretch justify-center">
          <button
            type="button"
        ...
        <div className="relative col-span-7 lg:col-span-9">
          <div className="grid grid-cols-10 gap-4 items-center">
            <div className={cn('space-y-2', !hasIdentity || isRadio ? 'col-span-10' : 'col-span-4')}>
              <input
                type="text"
                className="w-full bg-transparent border-none px-0 py-2.5 focus:ring-0 font-black text-slate-800 text-base uppercase placeholder:text-slate-400 tracking-tight"
              ...
              {!isRadio && hasIdentity && (
                <div className="flex flex-wrap items-center gap-2 animate-in fade-in slide-in-from-left-2">
                  <button
                    ...
                    className="bg-white/80 px-3 py-2 rounded-xl text-[10px] font-black text-primary uppercase tracking-wide border border-slate-100 hover:border-primary/20 hover:shadow-sm transition-all flex items-center gap-1.5"
                  ...
                  {drug.forme.startsWith('AUTRE') && (
                    <input
                      type="text"
                      className="w-28 bg-white/50 border border-slate-200 px-3 py-2 rounded-xl focus:ring-0 text-[10px] font-black text-slate-700 uppercase tracking-wide placeholder:text-slate-400 focus:border-primary/40 transition-colors"
                  ...
                  <div className="flex items-center gap-1.5 bg-white/80 px-3 py-2 rounded-xl border border-slate-100 shadow-sm">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wide">Dose</span>
                    <input
                      type="text"
                      className="w-20 bg-transparent border-none p-0 focus:ring-0 text-[11px] font-black text-slate-600 uppercase tracking-wide placeholder:text-slate-400"
                      placeholder="500MG"
                      value={drug.dosage}
                  ...
                  <button
                    ...
                    className={cn(
                      'px-3 py-2 rounded-xl border text-[10px] font-black uppercase tracking-wide transition-all select-none',
                      drug.non_substituable
                        ? 'bg-violet-600 text-white border-violet-600 shadow-sm shadow-violet-500/20'
                        : 'bg-white/80 text-slate-400 border-slate-100 hover:border-slate-300 hover:text-slate-600',
                    )}
            ...
            {!isRadio && hasIdentity && (
              <div className="col-span-6 relative h-full animate-in fade-in slide-in-from-right-2">
                <div className="bg-slate-50/50 px-4 py-3.5 rounded-2xl border border-slate-100 group-hover:bg-white transition-all focus-within:ring-2 focus-within:ring-primary/5 focus-within:border-primary/20 focus-within:shadow-sm">
                  <textarea
                    rows={2}
                    className="w-full bg-transparent border-none p-0 text-xs font-bold text-slate-600 focus:ring-0 resize-none placeholder:text-slate-300 leading-relaxed min-h-[2.75rem]"
          ...
          {!hasIdentity && !isRadio && (
            <p className="mt-1 text-[10px] font-semibold text-slate-400">
              Les détails de dose, forme et posologie apparaissent après identification du médicament.
            </p>
          )}
              ...
              >
                {suggestions.medications.map((m, i) => (
                  <button
                    ...
                    className={cn(
                      'w-full px-5 py-3 text-left text-xs font-black text-slate-600 transition-colors flex items-center justify-between',
