          ...
          <AnimatePresence>
            {items.map((item) => (
              <motion.div 
                ...
                exit={{ opacity: 0, x: 10 }}
                key={item.id} 
                className="grid grid-cols-12 gap-4 items-center bg-white/40 p-4 rounded-[2.2rem] border border-white/60 hover:bg-white hover:border-primary/20 hover:shadow-[0_20px_60px_rgba(0,51,128,0.06)] transition-all group relative overflow-hidden"
              >
                <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-primary/0 group-hover:bg-primary transition-all duration-500" />
                <div className="col-span-6 relative">
                  ...
                  <input
                    type="text"
                    className={cn(inputClass, "pl-12 border-transparent bg-transparent shadow-none font-black text-primary placeholder:text-slate-300")}
                    style={{ color: 'var(--primary)' }}
                    placeholder="Désignation de l'acte médical..."
                ...
                <div className="col-span-3">
                  <div className="relative">
                    <input
                      type="number"
                      className={cn(inputClass, "text-right font-black text-lg pr-12 border-transparent bg-slate-50/50 shadow-none")}
                      style={{ color: 'var(--primary)' }}
                      value={item.price}
