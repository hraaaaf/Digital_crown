...
export const CatalogTab: React.FC = () => {
  ...
  const handleAddSpecialty = () => {
    ...
    if (name) createSpecialty({ name, color: '#3B82F6' });
  };

  const handleAddPathology = (specialtyId: number) => {
    const name = prompt('Nom de la pathologie :');
    if (name) createPathology(specialtyId, { name });
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="bg-slate-900 text-white px-5 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-black transition-all shadow-xl shadow-slate-900/20"
        >
      ...
      <div className="flex flex-col xl:flex-row gap-8">
        ...
        <div className="w-full xl:w-1/3 flex flex-col gap-3">
          {specialties.map(spec => (
            <button
              ...
              className={cn(
                "w-full text-left p-5 rounded-[1.5rem] border transition-all duration-300 group flex items-center justify-between",
                (activeSpecialty?.id === spec.id) 
                  ? "bg-slate-900 text-white border-slate-900 shadow-2xl shadow-slate-900/20" 
                  : "bg-white border-slate-100 hover:border-slate-300 hover:shadow-lg text-slate-700"
              )}
            >
              <div className="flex items-center gap-4">
                <div className={cn(
                  "p-3 rounded-2xl transition-colors", 
                  (activeSpecialty?.id === spec.id) ? "bg-white/10" : "bg-slate-50 group-hover:bg-slate-100"
                )}>
                ...
                <div>
                  ...
                  <p className={cn("text-xs font-semibold mt-1", (activeSpecialty?.id === spec.id) ? "text-slate-400" : "text-slate-500")}>
                    {spec.acts.length} actes • {spec.pathologies.length} pathol.
                  </p>
                </div>
              </div>
            </button>
        ...
        {/* Détails de la Spécialité (Actes & Pathologies) */}
        {activeSpecialty && (
          <div className="flex-1 space-y-6">
            ...
            <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-100/50 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-6 relative">
                ...
                <button onClick={() => handleAddAct(activeSpecialty.id)} className="bg-sky-50 text-sky-600 px-4 py-2 rounded-xl font-bold hover:bg-sky-100 transition-colors text-sm flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 relative">
                ...
                {activeSpecialty.acts.length === 0 && (
                  <div className="col-span-full py-8 text-center text-slate-400 text-sm font-medium">
                    Aucun acte dans cette spécialité
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-100/50 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-6 relative">
                ...
                <button onClick={() => handleAddPathology(activeSpecialty.id)} className="bg-rose-50 text-rose-600 px-4 py-2 rounded-xl font-bold hover:bg-rose-100 transition-colors text-sm flex items-center gap-2">
                  <Plus size={16} /> Ajouter
                </button>
              </div>
              <div className="flex flex-wrap gap-2 relative">
                {activeSpecialty.pathologies.map(path => (
                  <div key={path.id} className="bg-white border border-slate-200 text-slate-600 px-4 py-2.5 rounded-xl shadow-sm font-bold text-sm hover:border-rose-200 hover:text-rose-600 transition-colors cursor-default">
                    {path.name}
                  </div>
                ))}
                {activeSpecialty.pathologies.length === 0 && (
                  <div className="w-full py-8 text-center text-slate-400 text-sm font-medium">
                    Aucune pathologie dans cette spécialité
                  </div>
                )}
              </div>
            </div>
