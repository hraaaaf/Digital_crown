...
import { Clock, Calendar, Ticket, AlertCircle } from 'lucide-react';
    ...
    <div className="space-y-12">
      ...
      <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm space-y-8">
        ...
        <div className="flex items-center gap-4">
          <input
            ...
            className="w-6 h-6 rounded-xl border-slate-200 text-primary focus:ring-primary"
          />
          <label htmlFor="is_continuous" className="font-bold text-slate-700">Journée Continue (Sans pause de midi)</label>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <input type="time" name="opening_time_morning" value={settings.opening_time_morning} onChange={handleChange} className="w-full bg-slate-50 border-none rounded-xl p-4 font-bold text-slate-700" />
              <span className="font-bold text-slate-400">à</span>
              <input type="time" name="closing_time_morning" value={settings.closing_time_morning} onChange={handleChange} className="w-full bg-slate-50 border-none rounded-xl p-4 font-bold text-slate-700" />
            </div>
          </div>
          ...
          {!settings.is_continuous && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                <input type="time" name="opening_time_afternoon" value={settings.opening_time_afternoon} onChange={handleChange} className="w-full bg-slate-50 border-none rounded-xl p-4 font-bold text-slate-700" />
                <span className="font-bold text-slate-400">à</span>
                <input type="time" name="closing_time_afternoon" value={settings.closing_time_afternoon} onChange={handleChange} className="w-full bg-slate-50 border-none rounded-xl p-4 font-bold text-slate-700" />
              </div>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm space-y-8">
        ...
        <div className="flex gap-4">
          <label className={`flex-1 border-2 p-6 rounded-2xl cursor-pointer transition-all ${settings.agenda_mode === 'EXACT' ? 'border-primary bg-primary/5' : 'border-slate-100'}`}>
          ...
          <label className={`flex-1 border-2 p-6 rounded-2xl cursor-pointer transition-all ${settings.agenda_mode === 'BLOCK' ? 'border-primary bg-primary/5' : 'border-slate-100'}`}>
      ...
      <div className="flex justify-end">
        <button
          onClick={handleSave}
          disabled={saving}
          className="bg-primary text-white font-black px-12 py-4 rounded-2xl hover:bg-black transition-all shadow-xl shadow-primary/20"
        >
