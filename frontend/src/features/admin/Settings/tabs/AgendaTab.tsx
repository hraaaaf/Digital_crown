...
export const AgendaTab: React.FC = () => {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setReadError("Impossible de charger les horaires réels du cabinet. Aucune configuration ne peut être modifiée tant que la lecture n'a pas réussi.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setSettings(prev => ({ ...prev, [name]: type === 'checkbox' ? (e.target as HTMLInputElement).checked : value }));
  };
  ...
  return (
    <div className="space-y-12 min-w-0">
      <div>
        <h2 className="text-2xl sm:text-3xl font-black text-slate-900 flex items-center gap-3 sm:gap-4"><Calendar className="text-primary shrink-0" size={32} />Horaires & Agenda</h2>
        <p className="text-slate-500 font-medium mt-2">Configurez votre emploi du temps et le mode de fonctionnement de votre cabinet.</p>
      </div>

      <div className="bg-white rounded-[2rem] p-5 sm:p-8 border border-slate-100 shadow-sm space-y-8 min-w-0">
        <h3 className="text-xl font-bold flex items-center gap-3"><Clock className="text-blue-500 shrink-0" />Horaires d'Ouverture</h3>
        <div className="flex items-start sm:items-center gap-4">
          <input type="checkbox" id="is_continuous" name="is_continuous" checked={settings.is_continuous} onChange={handleChange} className="w-6 h-6 shrink-0 rounded-xl border-slate-200 text-primary focus:ring-primary" />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 xl:gap-8 min-w-0">
          <div className="space-y-4 min-w-0">
            <h4 className="font-bold text-slate-500 uppercase tracking-widest text-xs">{settings.is_continuous ? "Heure d'ouverture" : 'Matin'}</h4>
      ...
      <div className="bg-white rounded-[2rem] p-5 sm:p-8 border border-slate-100 shadow-sm space-y-8 min-w-0">
        <h3 className="text-xl font-bold flex items-center gap-3"><Calendar className="text-purple-500 shrink-0" />Mode d'Agenda</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
          <label className={`min-w-0 border-2 p-5 sm:p-6 rounded-2xl cursor-pointer transition-all ${settings.agenda_mode === 'EXACT' ? 'border-primary bg-primary/5' : 'border-slate-100'}`}>
            ...
            <h4 className="font-black text-lg mb-2">Mode Précis (Pointu)</h4><p className="text-sm text-slate-500 font-medium">Créneaux horaires stricts (ex: 09:30, 10:00).</p>
          </label>
          <label className={`min-w-0 border-2 p-5 sm:p-6 rounded-2xl cursor-pointer transition-all ${settings.agenda_mode === 'BLOCK' ? 'border-primary bg-primary/5' : 'border-slate-100'}`}>
            ...
            <h4 className="font-black text-lg mb-2">Mode Souple (Blocs)</h4><p className="text-sm text-slate-500 font-medium">Les patients sont convoqués par demi-journée.</p>
          </label>
        </div>
      </div>

      <div className="bg-white rounded-[2rem] p-5 sm:p-8 border border-slate-100 shadow-sm space-y-8 min-w-0">
        <h3 className="text-xl font-bold flex items-center gap-3"><Ticket className="text-amber-500 shrink-0" />File d'attente (Tickets)</h3>
        <div className="flex items-start gap-4 min-w-0">
          <input type="checkbox" id="use_tickets" name="use_tickets" checked={settings.use_tickets} onChange={handleChange} className="w-6 h-6 shrink-0 rounded-xl border-slate-200 text-primary focus:ring-primary" />
          <div className="min-w-0"><label htmlFor="use_tickets" className="font-bold text-slate-700 block text-lg">Activer les tickets patients</label><p className="text-slate-500 font-medium text-sm mt-1">Permet d'attribuer un numéro (1, 2, 3...) aux patients qui arrivent sans RDV.</p></div>
        </div>
      </div>

      <div className="flex justify-stretch sm:justify-end">
        <button onClick={handleSave} disabled={saving} className="w-full sm:w-auto bg-primary text-white font-black px-6 sm:px-12 py-4 rounded-2xl hover:bg-black transition-all shadow-xl shadow-primary/20">{saving ? 'Sauvegarde...' : 'Sauvegarder Configuration'}</button>
      </div>
    </div>
  );
};
