...
const PERFORMANCE_PANEL_ID = 'dashboard-weekly-performance-panel';
  ...
  return (
    <motion.section variants={dashboardItemVariants} className="lg:col-span-2 space-y-5">
      <button
        ...
        aria-controls={PERFORMANCE_PANEL_ID}
        className="min-h-11 w-full text-[10px] font-black text-text-muted uppercase tracking-widest mb-2 px-4 flex items-center justify-between hover:text-primary transition-colors"
      >
      ...
      <div data-tour="dashboard-stats" className="bg-card-bg/85 backdrop-blur-xl rounded-elite-lg border border-border-main shadow-elite relative overflow-hidden">
        <AnimatePresence initial={false}>
          {expanded && (
            ...
            >
              {stats === null ? (
                <div role="status" className="p-8 h-[260px] flex flex-col items-center justify-center text-center">
                  ...
                  <h4 className="text-lg font-black text-primary font-outfit">Performance indisponible</h4>
                  <p className="text-xs font-medium text-text-muted mt-2 max-w-[300px] leading-relaxed">
                    Les statistiques n'ont pas pu être vérifiées. Aucun zéro de remplacement n'est affiché.
                  </p>
                </div>
              ) : (
                <div className="p-8 h-[410px] flex flex-col justify-between relative">
                  ...
                  <div className="relative z-10 flex items-center justify-between border-b border-border-main pb-4">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="text-right">
                        <span className="text-[8px] font-black text-text-muted uppercase tracking-wider block">Analyses / Patient</span>
                    ...
                    {stats.weekly_activity?.map((value, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          tabIndex={0}
                          aria-label={`${label || `Jour ${index + 1}`} : ${patientCount ?? 'nombre non disponible'} nouveaux dossiers`}
                          className="flex-1 flex flex-col items-center gap-2 group/bar relative z-10 h-full justify-end rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
                        >
                          {patientCount !== undefined && (
                            <div className="absolute bottom-[calc(100%-10px)] left-1/2 -translate-x-1/2 opacity-0 group-hover/bar:opacity-100 group-focus/bar:opacity-100 transition-all duration-300 pointer-events-none bg-slate-900/95 dark:bg-black/90 border border-border-main text-white px-2.5 py-1.5 rounded-xl text-[9px] font-black shadow-xl whitespace-nowrap mb-2 z-[20]">
                          ...
                          <div className="w-full relative rounded-t-xl overflow-hidden bg-slate-100 dark:bg-white/5 border border-transparent group-hover/bar:border-primary/20 group-focus/bar:border-primary/20 transition-all h-full flex items-end">
                            <motion.div
                              initial={{ height: 0 }}
                          ...
                          <span className="text-[9px] font-black text-text-muted uppercase tracking-wider group-hover/bar:text-primary group-focus/bar:text-primary transition-colors">
                            {label}
                          </span>
                        </div>
                      );
                  ...
                  <div className="relative z-10 border-t border-border-main pt-4 flex items-center justify-between text-[9px] font-bold text-text-muted uppercase tracking-wider">
                    <span className="flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" aria-hidden="true" /> Nouveaux dossiers / 7 jours
                    </span>
