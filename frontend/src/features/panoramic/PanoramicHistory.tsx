...
export const PanoramicHistory: React.FC<PanoramicHistoryProps> = ({ patientId, onSelect, onDelete }) => {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      setError(false);
      if (view === 'trash') {
        const response = await api.get(`/ia/patients/${patientId}/panoramic-trash`);
        setAnalyses(response.data);
      } else {
        const [activeResponse, trashResponse] = await Promise.all([
          api.get(`/ia/patients/${patientId}/panoramic-analyses`),
          api.get(`/ia/patients/${patientId}/panoramic-trash`),
        ]);
        const trashedIds = new Set<number>((trashResponse.data as PanoramicAnalysis[]).map(item => item.id));
        setAnalyses((activeResponse.data as PanoramicAnalysis[]).filter(item => !trashedIds.has(item.id)));
      }
    } catch (fetchError) {
    ...
    <div className="space-y-4">
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-200">
          ...
          <h3 className="text-lg font-black text-slate-400 uppercase tracking-tight">{view === 'active' ? 'Aucun historique' : 'Corbeille vide'}</h3>
          <p className="text-sm text-slate-400 mt-2">{view === 'active' ? 'Commencez par importer une première radiographie.' : 'Aucun examen panoramique supprimé à restaurer.'}</p>
        </div>
      ) : (
                ...
                <div className="flex min-w-0 items-center gap-4 sm:gap-6">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-1">
                      <span className="flex items-center gap-1.5 text-xs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg"><Activity size={14} className="text-indigo-500" />{landmarksCount} repère{landmarksCount > 1 ? 's' : ''} dentaire{landmarksCount > 1 ? 's' : ''}</span>
                      <span className="flex items-center gap-1.5 text-xs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg"><FileText size={14} className="text-emerald-500" /> Rapport enregistré</span>
                    </div>
                  </div>
                </div>
