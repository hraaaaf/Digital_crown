...
export const PanoramicStudio: React.FC<PanoramicStudioProps> = ({ patientId, patientName }) => {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (responseData.vision?.is_simulation || responseData.vision?.simulation_warning) {
        toast.error(responseData.vision?.simulation_warning || "Repérage automatique indisponible : vérification manuelle requise.");
      }
      setAnnotations([]);
  ...
  const handleSaveReport = async (editedMarkdown: string) => {
    if (!result?.id) return;
    try {
      const response = await api.put(`/ia/panoramic/${result.id}/report`, { report_narrative: editedMarkdown });
      setResult((prev: any) => ({ ...prev, report_narrative: response.data.report_narrative }));
  ...
  const handleImageClick = (e: React.MouseEvent) => {
    ...
    if (text) setAnnotations(prev => [...prev, { id: Date.now(), x, y, text }]);
    setToolMode('select');
  };
    ...
    <div className="flex flex-col lg:flex-row h-auto min-h-screen lg:h-full gap-4 lg:gap-6 bg-slate-50/50 p-4 lg:p-6 overflow-y-auto lg:overflow-hidden">
      <div className="flex-1 flex flex-col gap-4 overflow-hidden min-h-[50vh] lg:min-h-0">
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 p-4 lg:p-6 flex flex-col lg:flex-row items-start lg:items-center justify-between shrink-0 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 lg:gap-3 w-full lg:w-auto">
            ...
            <button
              onClick={() => { setViewMode('studio'); fileInputRef.current?.click(); }}
              disabled={loading}
          ...
          {viewMode === 'history' ? (
            <div className="absolute inset-0 bg-white overflow-y-auto p-8">
              <PanoramicHistory
                ...
                onDelete={(deletedId) => {
                  ...
                  if (compareResult?.id === deletedId) setCompareResult(null);
                }}
              />
            </div>
          ) : (
            <>
                ...
                <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-lg z-50 flex flex-col items-center justify-center overflow-hidden">
                  ...
                  <div className="relative flex flex-col items-center">
                    <div className="relative w-32 h-32 mb-8">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center"><Scan size={40} className="text-indigo-400 animate-pulse" /></div>
              ...
              {result && result.file_url && (
                <div
                  className={cn("relative w-full h-full p-8 flex gap-4 items-center justify-center group", toolMode === 'annotate' ? "cursor-crosshair" : "cursor-none")}
                  ref={containerRef}
                ...
                >
                  {compareMode && compareResult && (
                    <div className="flex-1 h-full flex flex-col gap-2">
                      ...
                      <div className="flex-1 relative rounded-xl border-2 border-slate-200 overflow-hidden bg-black"><img src={compareImageSrc || compareResult.file_url} className="w-full h-full object-contain opacity-80" /></div>
                    </div>
                  )}

                  <div
                    className={cn("relative flex items-center justify-center shadow-2xl rounded-lg overflow-hidden", compareMode ? "flex-1 h-full" : "w-full h-full")}
                    style={{ maxHeight: '100%', maxWidth: '100%', aspectRatio: !compareMode && imgSize ? `${imgSize.w} / ${imgSize.h}` : 'auto' }}
                  >
                    {compareMode && <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 text-[10px] font-black text-white bg-indigo-600 px-3 py-1 rounded-full uppercase tracking-widest shadow-lg">Examen Actuel (T1)</div>}
                    <XRayCanvas
                      imgUrl={mainImageSrc || result.file_url}
                      visionData={result.vision?.detections_data || null}
                      imgSize={imgSize}
                      onImgLoad={(e) => setImgSize({ w: e.currentTarget.naturalWidth, h: e.currentTarget.naturalHeight })}
                    ...
                    {imgSize && (
                      <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox={`0 0 ${imgSize.w} ${imgSize.h}`}>
                        {annotations.map((ann) => (
                          <motion.g key={ann.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                            <circle cx={(ann.x * imgSize.w) / 100} cy={(ann.y * imgSize.h) / 100} r={Math.max(imgSize.w * 0.008, 10)} fill="rgba(239, 68, 68, 0.8)" stroke="white" strokeWidth="2" />
                            <foreignObject x={(ann.x * imgSize.w) / 100 + 15} y={(ann.y * imgSize.h) / 100 - 15} width="200" height="100">
                              <div className="bg-slate-900/90 backdrop-blur-sm text-white p-2 rounded-lg text-[12px] font-bold border border-white/20 shadow-xl inline-block max-w-[180px] leading-tight">
                                {ann.text}
                                <button onClick={(e) => { e.stopPropagation(); setAnnotations(prev => prev.filter(a => a.id !== ann.id)); }} className="ml-2 text-red-400 hover:text-red-300"><XCircle size={12} className="inline" /></button>
                              </div>
                            </foreignObject>
                          </motion.g>
                        ))}
                  ...
                  <AnimatePresence>
                    {magnifierEnabled && magnifier.show && result && (
                      <motion.div
                        ...
                        style={{ width: 200, height: 200, left: `${magnifier.x}%`, top: `${magnifier.y}%`, transform: 'translate(-50%, -50%)', boxShadow: '0 0 40px rgba(0,0,0,0.5), inset 0 0 20px rgba(0,0,0,0.3)' }}
                      >
                        <div className="absolute w-full h-full" style={{ backgroundImage: `url(${mainImageSrc || result.file_url})`, backgroundRepeat: 'no-repeat', backgroundSize: '800%', backgroundPosition: `${magnifier.x}% ${magnifier.y}%`, filter: filterString }} />
                        <div className="absolute inset-0 flex items-center justify-center"><div className="w-full h-[1px] bg-indigo-500/50" /><div className="h-full w-[1px] bg-indigo-500/50" /><div className="absolute w-4 h-4 border border-indigo-400 rounded-full" /></div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <div className="absolute bottom-4 lg:bottom-8 left-1/2 -translate-x-1/2 bg-slate-800/90 backdrop-blur-md px-4 py-2 lg:px-6 lg:py-3 rounded-2xl border border-white/10 shadow-2xl flex flex-wrap justify-center items-center gap-2 lg:gap-6 z-40 w-[95%] lg:w-auto max-w-md lg:max-w-none">
                    <div className="flex items-center gap-3"><Sun size={16} className="text-amber-400" /><input type="range" min="50" max="200" value={imgFilters.brightness} onChange={(e) => setImgFilters(f => ({ ...f, brightness: Number(e.target.value) }))} className="w-16 lg:w-24 accent-amber-400 h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer" /></div>
                    <div className="w-[1px] h-6 bg-white/10" />
                    <div className="flex items-center gap-3"><Contrast size={16} className="text-indigo-400" /><input type="range" min="50" max="200" value={imgFilters.contrast} onChange={(e) => setImgFilters(f => ({ ...f, contrast: Number(e.target.value) }))} className="w-16 lg:w-24 accent-indigo-400 h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer" /></div>
                    <div className="w-[1px] h-6 bg-white/10" />
                    <div className="flex items-center gap-2">
                      <button onClick={() => setImgFilters(f => ({ ...f, invert: !f.invert }))} className={cn("p-2 rounded-lg transition-all", imgFilters.invert ? "bg-indigo-500 text-white" : "text-slate-400 hover:text-white")} title="Inverser les couleurs (Négatif)"><FlipHorizontal size={18} /></button>
                      <button onClick={() => setMagnifierEnabled(!magnifierEnabled)} className={cn("p-2 rounded-lg transition-all", magnifierEnabled ? "bg-indigo-500 text-white" : "text-slate-400 hover:text-white")} title="Loupe contextuelle"><Search size={18} /></button>
                      <button onClick={() => setToolMode(toolMode === 'annotate' ? 'select' : 'annotate')} className={cn("p-2 rounded-lg transition-all", toolMode === 'annotate' ? "bg-red-500 text-white" : "text-slate-400 hover:text-white")} title="Ajouter un détail clinique"><Type size={18} /></button>
                      <button onClick={() => setImgFilters({ brightness: 100, contrast: 110, invert: false })} className="p-2 text-slate-400 hover:text-white transition-all" title="Réinitialiser les filtres"><RefreshCcw size={18} /></button>
                    </div>
                  </div>
                </div>
              )}
          ...
          {result ? (
            <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col relative">
              <div className="sticky top-0 z-[50] bg-white/90 backdrop-blur-md border-b border-slate-100 p-2 flex gap-2 shrink-0">
                <button onClick={() => setSidebarTab('diagnostics')} className={cn("flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all", sidebarTab === 'diagnostics' ? "bg-indigo-600 text-white shadow-md shadow-indigo-200" : "bg-transparent text-slate-400 hover:bg-slate-50")}>Constatations</button>
                <button onClick={() => setSidebarTab('report')} className={cn("flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all", sidebarTab === 'report' ? "bg-indigo-600 text-white shadow-md shadow-indigo-200" : "bg-transparent text-slate-400 hover:bg-slate-50")}>Bilan PDF</button>
                <button onClick={() => setSidebarTab('evolution')} className={cn("flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all", sidebarTab === 'evolution' ? "bg-indigo-600 text-white shadow-md shadow-indigo-200" : "bg-transparent text-slate-400 hover:bg-slate-50")}>Évolution</button>
              </div>

              {sidebarTab === 'evolution' ? (
                <div className="p-6 space-y-5 flex-1 flex flex-col">
                  {evolutionLoading ? (
                    <div className="flex-1 flex items-center justify-center"><Loader2 size={24} className="animate-spin text-indigo-400" /></div>
                  ) : !evolutionData?.available ? (
                    <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center">
                      <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center"><TrendingUp size={22} className="text-slate-300" /></div>
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Évolution indisponible</p>
                      <p className="text-[10px] text-slate-400 max-w-[180px]">{evolutionData?.reason || 'Analysez au moins 2 bilans pour voir l\'évolution temporelle.'}</p>
                    </div>
                  ) : (
                    <>
                      <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-center">{evolutionData.older_date} → {evolutionData.newer_date}</div>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="bg-rose-50 border border-rose-100 rounded-2xl p-3 text-center"><div className="text-2xl font-black text-rose-600">{evolutionData.new_findings?.length ?? 0}</div><div className="text-[8px] font-bold text-rose-400 uppercase tracking-widest flex items-center justify-center gap-1 mt-0.5"><TrendingUp size={9} /> Nouvelles</div></div>
                        <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-3 text-center"><div className="text-2xl font-black text-emerald-600">{evolutionData.resolved_findings?.length ?? 0}</div><div className="text-[8px] font-bold text-emerald-400 uppercase tracking-widest flex items-center justify-center gap-1 mt-0.5"><CheckCircle2 size={9} /> Résolues</div></div>
                        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 text-center"><div className="text-2xl font-black text-amber-600">{evolutionData.worsened_findings?.length ?? 0}</div><div className="text-[8px] font-bold text-amber-400 uppercase tracking-widest flex items-center justify-center gap-1 mt-0.5"><AlertTriangle size={9} /> Aggravées</div></div>
                        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3 text-center"><div className="text-2xl font-black text-slate-500">{evolutionData.stable_findings?.length ?? 0}</div><div className="text-[8px] font-bold text-slate-400 uppercase tracking-widest flex items-center justify-center gap-1 mt-0.5"><Minus size={9} /> Stables</div></div>
                      </div>
                      {evolutionData.new_findings?.length > 0 && <div className="space-y-1.5"><h5 className="text-[8px] font-black text-rose-500 uppercase tracking-widest">Nouvelles constatations</h5>{evolutionData.new_findings.map((f: string, i: number) => <div key={i} className="text-[10px] text-slate-600 bg-rose-50 px-3 py-1.5 rounded-xl">{f}</div>)}</div>}
                      {evolutionData.worsened_findings?.length > 0 && <div className="space-y-1.5"><h5 className="text-[8px] font-black text-amber-500 uppercase tracking-widest">Éléments aggravés</h5>{evolutionData.worsened_findings.map((f: string, i: number) => <div key={i} className="text-[10px] text-slate-600 bg-amber-50 px-3 py-1.5 rounded-xl">{f}</div>)}</div>}
                      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4"><p className="text-[10px] font-semibold text-indigo-700 leading-relaxed">{evolutionData.summary_text}</p></div>
                    </>
                  )}
                </div>
              ) : sidebarTab === 'report' ? (
                ...
                <div className="p-8 space-y-8 flex-1 flex flex-col">
                  ...
                  {(annotations.length > 0 || manualAnomaliesList.length > 0) && (
                    <div className="space-y-4">
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2"><Type size={12} className="text-indigo-500" />Constatations ({manualAnomaliesList.length + annotations.length})</h4>
                      <div className="space-y-2">
                        {manualAnomaliesList.map((item, idx) => {
                          ...
                          return (
                            <div key={`manual-${idx}`} className={cn("border p-4 rounded-2xl flex justify-between items-center group transition-all", isUrgent ? "bg-rose-50/70 border-rose-200" : "bg-indigo-50/50 border-indigo-100")}>
                              <div className="flex items-center gap-2 flex-1 min-w-0">
                                <span className={cn("font-black text-sm shrink-0", isUrgent ? "text-rose-900" : "text-indigo-900")}>Dent {item.fdi}</span>
                                <span className="text-xs font-bold text-slate-600 truncate">{label}</span>
                                {isUrgent && <span className="shrink-0 text-[8px] font-black text-rose-600 bg-rose-100 border border-rose-200 px-1.5 py-0.5 rounded-full uppercase tracking-wider">Urgent</span>}
                              </div>
                              <button onClick={() => toggleAnomaly(item.fdi, item.anomaly)} className="text-red-400 hover:text-red-600 transition-all p-1 bg-white rounded-full shadow-sm opacity-0 group-hover:opacity-100 shrink-0"><Trash2 size={14} /></button>
                            </div>
                          );
                        })}
                        {annotations.map((ann) => (
                  ...
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2"><Activity size={12} className="text-amber-500" />Constats Généraux</h4>
                    <div className="flex flex-wrap gap-2">
                      {GLOBAL_FINDINGS.map((gf) => {
                        const active = globalFindings.includes(gf.id);
                        return <button key={gf.id} onClick={() => toggleGlobalFinding(gf.id)} className={cn("px-3 py-2 rounded-xl text-[10px] font-bold tracking-wide border transition-all", active ? "bg-amber-500 text-white border-amber-500 shadow-sm shadow-amber-100" : "bg-white text-slate-500 border-slate-200 hover:border-amber-300 hover:text-amber-600")} title={gf.group}>{gf.label}</button>;
                      })}
                    </div>
                  </div>

                  <div className="pt-4 mt-auto sticky bottom-0 bg-white pb-2 shrink-0">
                    <button onClick={async () => { await handleFinalize(); setSidebarTab('report'); }} disabled={loading} className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[1.5rem] font-black tracking-widest shadow-xl shadow-indigo-100 transition-all active:scale-95 flex items-center justify-center gap-3 group disabled:opacity-50">
                      {loading ? <Loader2 className="animate-spin" size={20} /> : <CheckCircle2 size={20} className="group-hover:scale-110 transition-transform" />}VALIDER ET GÉNÉRER
                    </button>
                  </div>
                </div>
              )}
            ...
            <div className="h-full flex flex-col items-center justify-center p-8">
              <h3 className="text-xs font-black text-slate-400 tracking-[0.2em] mb-12 flex items-center gap-2 uppercase"><ShieldAlert size={16} className="text-indigo-500" />Constatations cliniques</h3>
              <div className="flex flex-col items-center justify-center text-slate-300 gap-4 opacity-40">
                <div className="w-20 h-20 rounded-full bg-slate-50 flex items-center justify-center border-2 border-dashed border-slate-200"><Activity size={40} /></div>
      ...
      <AnimatePresence>
        {(previewPdfUrl || isPreviewLoading) && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-4 z-50 rounded-[3rem] overflow-hidden shadow-2xl ring-1 ring-black/5">
            <LivePreview pdfUrl={previewPdfUrl} loading={isPreviewLoading} onClose={() => { if (previewPdfUrl) URL.revokeObjectURL(previewPdfUrl); setPreviewPdfUrl(null); setIsPreviewLoading(false); }} title="Aperçu du Bilan Panoramique" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
