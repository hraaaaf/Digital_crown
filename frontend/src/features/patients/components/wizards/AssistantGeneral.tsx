...
interface AssistantGeneralProps {
...
const QUESTIONS = [
  {
    ...
    subtitle: "Raison de la venue du patient.",
    options: [
      { id: 'm_routine', label: "Contrôle de routine / Bilan", value: 0 },
      { id: 'm_urgence', label: "Urgence / Douleur aiguë", value: 1 },
      { id: 'm_esthetique', label: "Demande esthétique", value: 2 }
    ]
  },
  {
    id: 'hygiene',
    title: "Hygiène Bucco-Dentaire",
    subtitle: "Évaluation clinique de l'hygiène actuelle.",
    options: [
      { id: 'h_bonne', label: "Bonne (Brossage régulier, gencive saine)", value: 0 },
      { id: 'h_moyenne', label: "Moyenne (Plaque visible, tartre localisé)", value: 1 },
      { id: 'h_mediocre', label: "Médiocre (Tartre abondant, inflammation)", value: 2 }
    ]
  },
  {
    id: 'medical',
    title: "Antécédents Médicaux Pertinents",
    subtitle: "Alerte médicale majeure.",
    options: [
      { id: 'med_ras', label: "RAS (Patient sain)", value: 0 },
      { id: 'med_infect', label: "Haut Risque Infectieux (Cardiopathie, Prothèse)", value: 1 },
      { id: 'med_allergie', label: "Allergies Médicamenteuses (Pénicilline, etc.)", value: 2 }
    ]
  }
];

export const AssistantGeneral: React.FC<AssistantGeneralProps> = ({ onComplete, onCancel }) => {
  ...
  if (isCalculating) {
    return (
      <div className="flex flex-col items-center justify-center py-10 gap-4">
        ...
        <h3 className="text-sm font-black uppercase tracking-widest text-slate-500">Synthèse du Bilan Général...</h3>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div className="flex gap-1">
          {QUESTIONS.map((_, idx) => (
            <div key={idx} className={cn("h-1.5 rounded-full transition-all duration-300", idx <= currentStep ? "w-8 bg-emerald-500" : "w-4 bg-slate-200 dark:bg-slate-700")} />
          ))}
        </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isSelected 
                      ...
                      : "bg-card-bg border-border-main hover:border-emerald-400 hover:shadow-sm text-text-main"
                  )}
                >
                  <span className="font-bold">{opt.label}</span>
                  <div className={cn(
                    "w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors",
                    isSelected ? "border-emerald-500 bg-emerald-500 text-white" : "border-slate-300 group-hover:border-emerald-400"
                  )}>
