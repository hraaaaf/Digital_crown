...
export const PatientCompanionPanel = ({ patientId, patientEmail }: PatientCompanionPanelProps) => {
  ...
  return (
    <div className="space-y-5" data-patient-companion-admin>
      <section className="grid grid-cols-1 lg:grid-cols-[1.15fr_0.85fr] gap-4">
        <div className="bg-card-bg rounded-[1.75rem] border border-border-main shadow-elite p-5 md:p-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-main">Patient Companion</h2>
              <p className="mt-1 text-sm font-medium text-text-muted">Administration sécurisée de l’accès mobile, sans dupliquer le dossier patient.</p>
            </div>
      ...
      {ephemeralInvitation && (
        <section className="bg-card-bg rounded-[1.75rem] border border-primary/20 shadow-elite p-5 md:p-6" data-ephemeral-invitation>
          ...
          <div className="mt-5 grid grid-cols-1 md:grid-cols-[220px_1fr] gap-5 items-center">
            <div className="rounded-2xl border border-border-main bg-white p-3 flex items-center justify-center min-h-[220px]">{ephemeralInvitation.qrDataUrl ? <img src={ephemeralInvitation.qrDataUrl} alt="QR d’activation Patient Companion" className="w-48 h-48 object-contain" /> : <QrCode size={96} className="text-slate-300" />}</div>
      ...
      <section className="rounded-[1.75rem] border border-rose-200 bg-rose-50/70 p-5 md:p-6">
...
const StatusMetric = ({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: string; tone: 'good' | 'warn' | 'neutral' }) => (
  <div className={cn('rounded-2xl border px-4 py-3', tone === 'good' ? 'border-emerald-200 bg-emerald-50/70' : tone === 'warn' ? 'border-amber-200 bg-amber-50/70' : 'border-border-main bg-background')}><div className="flex items-center gap-2 text-text-muted">{icon}<span className="text-[10px] font-black uppercase tracking-widest">{label}</span></div><p className="mt-2 text-lg font-black text-main">{value}</p></div>
);
...
const ResourceCard = ({ title, icon, empty, children }: { title: string; icon: React.ReactNode; empty: string; children: React.ReactNode }) => {
  ...
  return <div className="bg-card-bg rounded-[1.75rem] border border-border-main shadow-elite p-5 md:p-6"><div className="flex items-center gap-2 text-primary">{icon}<h3 className="font-black text-main">{title}</h3></div><div className="mt-4 max-h-[420px] overflow-y-auto space-y-2 pr-1">{hasChildren ? children : <p className="py-8 text-center text-sm font-medium text-text-muted">{empty}</p>}</div></div>;
};
