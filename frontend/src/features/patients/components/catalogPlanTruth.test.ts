...
describe('catalog connected truth', () => {
  ...
  it('captures manual overrides by value and disambiguates the specialty', () => {
    const step = buildCatalogPlanStep(
      { id: 7, name: 'Détartrage', code: 'DET-001', basePrice: 500, specialtyName: 'Prévention' },
      'Détartrage complet',
      450.129,
      'Ajouté le 21/08/2026',
      2,
    );
    expect(step.title).toBe('Détartrage complet');
    expect(step.catalog_snapshot).toEqual({ act_id: 7, code: 'DET-001', name: 'Détartrage complet', price: 450.13 });
    expect(step.assistant).toContain('Prévention');
    expect(step.assistant).toContain('450,13 DH');
  ...
  it('preserves a persisted snapshot when rebuilding a plan payload', () => {
    const normalized = normalizePersistedPlanStep({
      ...
      assistant: 'Catalogue cabinet · Prévention · DET-001 · 450 DH · Tarif capturé',
      status: 'done',
      date_str: 'Fait le 21/08/2026',
