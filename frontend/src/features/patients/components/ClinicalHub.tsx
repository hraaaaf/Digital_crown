...
export const ClinicalHub: React.FC<ClinicalHubProps> = ({ patientId }) => {
  ...
  const addToPlan = async () => {
    ...
    try {
      ...
      toast.success('Acte ajouté au plan · tarif figé.');
    } catch (error) {
  ...
  return (
    <div className="w-full min-w-0 space-y-4">
      <section aria-label="Catalogue du cabinet vers plan de traitement" className="rounded-[1.5rem] border border-primary/15 bg-card-bg/90 p-4 shadow-sm backdrop-blur-xl sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-primary"><ShieldCheck size={15} /> Ajouter un acte au plan</span>
              <span className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-emerald-700">Tarif figé</span>
            </div>
            <p className="mt-1 max-w-2xl text-xs font-bold leading-relaxed text-text-muted">Choisissez un acte du catalogue du cabinet. Le nom et le tarif restent modifiables avant l’ajout ; une modification future dans Réglages ne changera pas ce plan.</p>
          </div>
