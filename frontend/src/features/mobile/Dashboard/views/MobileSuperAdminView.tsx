...
const SECTIONS: Array<{ key: SuperAdminSection; label: string }> = [
  ...
  { key: 'marketplace', label: 'Approvisionnement' },
      ...
      <main className="mx-auto max-w-4xl space-y-5 px-5 pb-12 pt-5">
        ...
        {section === 'marketplace' && (
          <MarketplaceSection
            ...
            onSync={(supplier, force) => requestConfirm({ title: force ? 'Forcer la synchronisation ?' : 'Synchroniser le fournisseur ?', body: `${supplier.name} sera synchronisé avec les données centrales.${force ? ' Les délais d’attente habituels seront ignorés.' : ''}`, confirmLabel: force ? 'Forcer' : 'Synchroniser', danger: force, action: () => superadmin.marketplaceActions.syncSupplier(supplier.id, force) })}
          />
        )}
  ...
  return (
    <div className="space-y-5" data-superadmin-overview>
      ...
      <section className="rounded-[22px] border border-glass-border bg-card p-4 shadow-sm">
        ...
        ) : <p className="mt-4 text-xs font-bold text-text-muted">Aucune donnée d’approvisionnement chargée.</p>}
      </section>
    </div>
  );
}
    ...
    <div className="space-y-5" data-superadmin-client-detail>
      ...
      <section className="rounded-[20px] border border-glass-border bg-card p-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          ...
          <button type="button" onClick={() => confirm({ title: 'Envoyer une relance ?', body: 'La relance sera envoyée via le canal disponible, actuellement WhatsApp si un téléphone est renseigné.', confirmLabel: 'Envoyer', action: () => props.actions.sendRenewal(props.client.id, 'Votre licence Digital Crown expire bientôt.') })} className="min-h-11 rounded-[14px] border border-glass-border bg-background text-[10px] font-black"><Mail size={14} className="mr-1 inline" />Relance</button>
        </div>
...
function MarketplaceSection(props: {
  ...
  if (props.locked) return <div className="space-y-4" data-superadmin-marketplace><SectionTitle eyebrow="Approvisionnement" title="Administration approvisionnement" /><div className="rounded-[22px] border border-amber-200 bg-amber-50 p-5"><Fingerprint size={28} className="text-amber-700" /><h3 className="mt-4 text-lg font-black text-amber-950">Vérification biométrique requise</h3><p className="mt-2 text-sm font-semibold leading-relaxed text-amber-800">Une vérification biométrique ouvre l’accès à l’administration de l’approvisionnement pendant 5 minutes.</p><button type="button" onClick={props.onUnlock} disabled={props.loading} className="mt-5 min-h-13 w-full rounded-[16px] bg-amber-600 text-xs font-black text-white disabled:opacity-50">{props.loading ? 'Vérification…' : 'Vérifier biométrie'}</button></div></div>;
  ...
  return <div className="space-y-4" data-superadmin-marketplace>
    ...
    {props.tab === 'incidents' && <div className="space-y-3">{props.incidents.length ? props.incidents.map((incident, index) => <div key={`${incident.supplierId}-${index}`} className="rounded-[18px] border border-rose-200 bg-rose-50 p-4"><div className="flex items-center gap-2"><AlertTriangle size={16} className="text-rose-700" /><p className="text-sm font-black text-rose-900">{incident.supplierName || `Fournisseur #${incident.supplierId}`}</p></div><p className="mt-2 text-[10px] font-bold text-rose-800">{statusLabel(incident.freshness.status)} · {incident.consecutiveFailures} échec(s)</p><p className="mt-1 text-[10px] font-semibold text-rose-700">{'Synchronisation à vérifier.'}</p></div>) : <Empty label="Aucun incident de synchronisation." />}</div>}
    {props.tab === 'audit' && <div className="space-y-2">{props.audit.length ? props.audit.map(event => <div key={event.id} className="rounded-[16px] border border-glass-border bg-card p-3"><div className="flex items-center justify-between gap-3"><span className="text-[10px] font-black">{statusLabel(event.action)}</span><span className="text-[8px] font-bold text-text-muted">{formatDate(event.createdAt)}</span></div><p className="mt-1 text-[9px] font-semibold text-text-muted">{statusLabel(event.entityType)} #{event.entityId} · Cabinet #{event.employerId}</p></div>) : <Empty label="Journal d'approvisionnement vide." />}</div>}
  </div>;
}
...
function SupplierCreateSheet(props: { onClose: () => void; onSubmit: (payload: SupplierCreateInput) => Promise<boolean> }) {
  ...
  const [busy, setBusy] = useState(false);
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); try { if (await props.onSubmit(form)) props.onClose(); } finally { setBusy(false); } };
  return <Sheet title="Nouveau fournisseur" subtitle="Création fournisseur" onClose={props.onClose}><form onSubmit={submit} className="space-y-3"><NumberField label="Cabinet cible (ID)" value={form.employerId} min={1} onChange={value => setForm({ ...form, employerId: value })} /><Field label="Identifiant fournisseur" required value={form.supplierKey} onChange={value => setForm({ ...form, supplierKey: value })} /><Field label="Nom" required value={form.name} onChange={value => setForm({ ...form, name: value })} /><Field label="Badge" value={form.badge || ''} onChange={value => setForm({ ...form, badge: value })} /><TextArea label="Description" value={form.description || ''} onChange={value => setForm({ ...form, description: value })} /><Field label="Adresse de connexion" value={form.apiBaseUrl || ''} onChange={value => setForm({ ...form, apiBaseUrl: value })} /><SelectField label="Mode de synchronisation" value={form.syncMode || 'manual'} options={['manual', 'api']} optionLabels={SYNC_MODE_LABELS} onChange={value => setForm({ ...form, syncMode: value })} /><button disabled={busy} className="min-h-13 w-full rounded-[16px] bg-primary text-xs font-black text-white">{busy ? 'Création…' : 'Créer'}</button></form></Sheet>;
}
...
function ProductCreateSheet(props: { suppliers: GlobalSupplier[]; onClose: () => void; onSubmit: (payload: ProductCreateInput) => Promise<boolean> }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); try { if (await props.onSubmit(form)) props.onClose(); } finally { setBusy(false); } };
  return <Sheet title="Nouveau produit" subtitle="Nouveau produit du catalogue" onClose={props.onClose}><form onSubmit={submit}><NumberField label="Cabinet cible (ID)" value={form.employerId} min={1} onChange={value => setForm({ ...form, employerId: value, supplierId: props.suppliers.find(item => item.employerId === value)?.id ?? 0 })} /><ProductFields form={form} setForm={setForm} suppliers={compatible} /><button disabled={busy} className="mt-4 min-h-13 w-full rounded-[16px] bg-primary text-xs font-black text-white">{busy ? 'Création…' : 'Créer'}</button></form></Sheet>;
}
...
function OperationsSection(props: { orders: OperationalOrder[]; globalOrders: ReturnType<typeof useMobileSuperAdmin>['globalOrders']; finance: ReturnType<typeof useMobileSuperAdmin>['financeSummary']; globalLocked: boolean; loading: boolean; onUnlockGlobal: () => void; onOpen: (order: OperationalOrder) => void }) {
  return <div className="space-y-5" data-superadmin-operations>
    ...
    <section className="space-y-3"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-primary">Cabinet sélectionné</p>{props.loading && !props.orders.length ? <Empty label="Chargement des commandes…" /> : props.orders.length ? props.orders.map(order => <button key={order.id} type="button" onClick={() => props.onOpen(order)} className="flex min-h-[84px] w-full items-center gap-3 rounded-[19px] border border-glass-border bg-card p-4 text-left shadow-sm"><div className="flex h-10 w-10 items-center justify-center rounded-[13px] bg-primary/10 text-primary"><ShoppingCart size={17} /></div><div className="min-w-0 flex-1"><p className="truncate text-xs font-black">{order.orderNumber}</p><p className="truncate text-[9px] font-bold text-text-muted">{order.partnerName} · {statusLabel(order.status)}</p><p className="mt-1 text-[10px] font-black text-primary">{formatMoney(order.currentTotal)}</p></div><ChevronRight size={15} /></button>) : <Empty label="Aucune commande pour ce cabinet." />}</section>
    <section className="rounded-[20px] border border-glass-border bg-card p-4"><div className="flex items-center justify-between"><div><p className="text-[9px] font-black uppercase tracking-[0.16em] text-primary">Vue multi-cabinets</p><h3 className="mt-1 text-sm font-black">Commandes multi-cabinets</h3></div><Shield size={18} className="text-primary" /></div>{props.globalLocked ? <button type="button" onClick={props.onUnlockGlobal} className="mt-4 min-h-11 w-full rounded-[14px] bg-amber-600 text-[10px] font-black text-white"><Fingerprint size={14} className="mr-1 inline" />Vérifier l’identité</button> : <div className="mt-3 space-y-2">{props.globalOrders.slice(0, 8).map(order => <div key={order.id} className="flex items-center justify-between gap-3 rounded-[13px] bg-background p-3"><div className="min-w-0"><p className="truncate text-[10px] font-black">{order.orderNumber}</p><p className="truncate text-[8px] font-bold text-text-muted">Cabinet #{order.employerId} · {order.supplierName}</p></div><div className="text-right"><p className="text-[9px] font-black">{statusLabel(order.status)}</p><p className="text-[8px] font-bold text-primary">{formatMoney(order.currentTotal)}</p></div></div>)}</div>}</section>
  </div>;
}
...
function OperationSheet(props: {
  ...
  return <Sheet title={props.order.orderNumber} subtitle={`${props.order.partnerName} · ${statusLabel(props.order.status)}`} onClose={props.onClose}><div className="space-y-5" data-superadmin-operation-detail>
    ...
    <section className="rounded-[20px] border border-glass-border bg-card p-4"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-primary">Cycle commande</p>{props.order.status === 'DRAFT' && <button type="button" onClick={() => props.confirm({ title: 'Envoyer la commande au fournisseur ?', body: `Commande ${props.order.orderNumber} → ${props.order.partnerName}. L’envoi sera confirmé après réponse du fournisseur.`, confirmLabel: 'Envoyer', danger: true, action: async () => { const ok = await props.dispatch(props.order.id); if (ok) reload(); return ok; } })} className="mt-3 min-h-12 w-full rounded-[15px] bg-primary text-xs font-black text-white"><Send size={15} className="mr-1 inline" />Envoyer au fournisseur</button>}{transitions.length > 0 && <div className="mt-3 grid grid-cols-2 gap-2">{transitions.map(state => <button key={state} type="button" onClick={() => props.confirm({ title: `Passer en ${statusLabel(state)} ?`, body: 'Le statut de la commande sera mis à jour après validation.', confirmLabel: statusLabel(state), danger: state === 'CANCELLED', action: () => props.updateOrder(props.order.id, { status: state }) })} className={`min-h-11 rounded-[13px] text-[9px] font-black ${state === 'CANCELLED' ? 'bg-rose-100 text-rose-700' : 'border border-glass-border bg-background text-text-main'}`}>{statusLabel(state)}</button>)}</div>}{detail?.dispatch && <div className="mt-4 rounded-[14px] bg-background p-3 text-[9px] font-semibold text-text-muted"><p className="font-black text-text-main">Envoi fournisseur</p><p className="mt-1">Résultat : {statusLabel(detail.dispatch.outcome)}</p><p>Référence fournisseur : {String(detail.dispatch.supplierReference ?? '—')}</p></div>}</section>

    <section className="rounded-[20px] border border-glass-border bg-card p-4"><div className="flex items-center gap-2"><Truck size={16} className="text-primary" /><p className="text-[9px] font-black uppercase tracking-[0.16em] text-primary">Suivi fournisseur</p></div><Field label="Référence fournisseur" value={supplierReference} onChange={setSupplierReference} /><Field label="Livraison attendue" type="datetime-local" value={expectedDeliveryAt} onChange={setExpectedDeliveryAt} /><TextArea label="Note" value={procurementNote} onChange={setProcurementNote} />{props.order.lines.map(line => <NumberField key={line.productId} label={`Reliquat ${line.sku} · max ${line.quantity}`} value={backorders[line.productId] || 0} min={0} max={line.quantity} onChange={value => setBackorders({ ...backorders, [line.productId]: value })} />)}<button type="button" disabled={!supplierReference.trim()} onClick={() => props.confirm({ title: 'Enregistrer l’accusé fournisseur ?', body: 'Les quantités confirmées et les reliquats seront vérifiés avant l’enregistrement.', confirmLabel: 'Enregistrer', action: async () => { const ok = await props.acknowledgeProcurement(props.order.id, { supplierReference, expectedDeliveryAt: expectedDeliveryAt || null, backorderedLines: Object.entries(backorders).filter(([, qty]) => qty > 0).map(([productId, quantityBackordered]) => ({ productId, quantityBackordered })), note: procurementNote || undefined }); if (ok) reload(); return ok; } })} className="mt-3 min-h-12 w-full rounded-[15px] bg-primary text-xs font-black text-white disabled:opacity-40">Accuser réception fournisseur</button></section>

    <section className="rounded-[20px] border border-glass-border bg-card p-4"><div className="flex items-center gap-2"><ReceiptText size={16} className="text-primary" /><p className="text-[9px] font-black uppercase tracking-[0.16em] text-primary">Facture & rapprochement</p></div>{detail?.reconciliation && <div className="mt-3 grid grid-cols-2 gap-2"><Metric label="Statut" value={statusLabel(detail.reconciliation.reconciliationStatus)} /><Metric label="Variance" value={formatMoney(detail.reconciliation.invoiceVariance)} /></div>}<Field label="Référence facture" value={invoiceReference} onChange={setInvoiceReference} /><NumberField label="Montant total MAD" value={invoiceAmount} min={0} step="0.01" onChange={setInvoiceAmount} /><button type="button" disabled={!invoiceReference.trim()} onClick={() => props.confirm({ title: 'Enregistrer cette facture fournisseur ?', body: `${invoiceReference} · ${formatMoney(invoiceAmount)}. La facture sera enregistrée après validation des informations.`, confirmLabel: 'Enregistrer facture', action: async () => { const ok = await props.recordInvoice(props.order.id, { invoiceReference, amountTotal: invoiceAmount }); if (ok) reload(); return ok; } })} className="mt-3 min-h-12 w-full rounded-[15px] bg-primary text-xs font-black text-white disabled:opacity-40">Enregistrer facture</button></section>
...
function SelectField(props: { label: string; value: string; options: string[]; optionLabels?: Record<string, string>; onChange: (value: string) => void }) {
