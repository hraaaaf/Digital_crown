...
export function MobileQuickDocumentSheet({ patient, preview = false, onClose }: { patient: PatientIdentity; preview?: boolean; onClose: () => void }) {
  ...
  const buildPayload = () => {
    ...
    if (type === 'devis' || type === 'honoraires') {
      ...
      return { type: type === 'honoraires' ? 'note' : 'devis', patient_id: patient.id, data: type === 'honoraires' ? { payments: entries, doc_date: docDate, teeth_data: [], installments: [], is_global_note: false } : { items: entries, doc_date: docDate, teeth_data: [], installments: [] }, is_accounted: true, payment_status: 'EN_ATTENTE' as const };
    }
    if (type === 'libre') {
  ...
  const generate = async () => {
    ...
    if (preview) { setSuccess('Aperçu isolé : document prêt à générer.'); return; }
    setBusy(true);
    try {
      ...
      const response = await mobileFetch(`${baseUrl}/api/documents/generate?archive=true&preview=false&force=false`, { method: 'POST', headers: { Authorization: `Bearer ${creds.access_token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload), signal: AbortSignal.timeout(20000) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) { const detail = body?.detail; throw new Error((typeof detail === 'string' ? detail : detail?.message) || `Génération indisponible (${response.status}).`); }
    ...
    } catch (failure) { setError(failure instanceof Error ? failure.message : 'Impossible de générer le document.'); } finally { setBusy(false); }
  };
  ...
  return <div data-mobile-quick-document className="fixed inset-0 z-[80] bg-black/35 backdrop-blur-sm flex items-end sm:items-center justify-center">
    <section className="w-full max-w-2xl max-h-[92dvh] overflow-y-auto rounded-t-[30px] sm:rounded-[30px] border border-glass-border bg-card p-4 sm:p-5 shadow-2xl" style={{ backgroundColor: 'var(--glass-bg)' }}>
      ...
      {!type ? <div className="space-y-2">{TYPES.map((entry) => <button key={entry.value} type="button" onClick={() => setType(entry.value)} className="w-full min-h-16 rounded-[20px] border border-glass-border bg-background px-4 py-3 text-left active:scale-[0.99] transition-transform"><p className="text-sm font-black text-text-main">{entry.label}</p><p className="mt-1 text-[11px] font-bold text-text-muted">{entry.hint}</p></button>)}</div> : <div className="space-y-4">
        ...
        <button type="button" onClick={() => void generate()} disabled={busy} className="w-full min-h-14 rounded-[18px] bg-primary text-primary-foreground text-sm font-black flex items-center justify-center gap-2 disabled:opacity-50">{busy ? <Loader2 size={17} className="animate-spin" /> : <FileText size={17} />} Générer et archiver</button>
