...
function normalizeAppointmentTime(time: string | null | undefined): string {
      ...
      <div className="space-y-6">
          ...
          {(['jour', 'semaine', 'mois'] as ViewMode[]).map(mode => (
            <button
              ...
              className={cn(
                ...
                viewMode === mode ? 'bg-primary text-white shadow-md scale-[1.02]' : 'text-text-muted hover:text-primary'
              )}
            >
              {mode}
            </button>
        ...
        {viewMode === 'mois' && (
          <div className="bg-white border border-slate-100 rounded-[24px] p-4 shadow-sm">
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              {Array.from({ length: (weekDays[0].getDay() + 6) % 7 }).map((_, i) => <div key={`empty-${i}`} />)}
              {monthDays.map(d => {
                const dateStr = format(d, 'yyyy-MM-dd');
                return (
                  <DroppableDay
            ...
            <p className="text-[10px] text-center text-text-muted mt-4">Glissez-déposez un rendez-vous sur un jour pour le déplacer.</p>
          </div>
        )}
        ...
        {totalCount > 0 && viewMode === 'jour' && (
          <div className="bg-glass-bg border border-glass-border backdrop-blur-md rounded-[20px] p-4 shadow-elite">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-black text-text-muted uppercase tracking-widest flex items-center gap-1.5"><Clock size={12} /> Progression du jour</span>
            ...
            <div className="h-1.5 bg-border-main rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-primary to-secondary rounded-full transition-all duration-700" style={{ width: `${(termineCount / totalCount) * 100}%` }} />
            </div>
          </div>
        )}
        ...
        {syncStatus === 'loading' && !snapshot ? (
          ...
          <div className="bg-card border border-border-main rounded-[24px] py-20 text-center shadow-elite relative overflow-hidden mt-4">
            <div className="w-16 h-16 bg-amber-500/5 border border-amber-500/10 rounded-[20px] flex items-center justify-center mx-auto mb-4 text-amber-500"><WifiOff size={32} /></div>
          ...
          <div className="bg-card border border-border-main rounded-[24px] py-20 text-center shadow-elite relative overflow-hidden mt-4">
            <div className="w-16 h-16 bg-primary/5 border border-primary/10 rounded-[20px] flex items-center justify-center mx-auto mb-4 text-primary"><Calendar size={32} /></div>
                ...
                {timelineSlots.map(time => {
                  ...
                  if (appointmentsAtTime.length > 0) {
                    return (
                      <div key={time} className="relative">
                        <div className="absolute -left-10 mt-3 bg-white px-1 text-[10px] font-bold text-slate-500 w-8 text-right z-10">{time}</div>
                        <div className="absolute left-[-19px] top-4 w-2.5 h-2.5 rounded-full border-2 border-primary bg-white z-10" />
                        <div className="space-y-3">
                          {appointmentsAtTime.map(appointment => (
                            <DraggableApptCard key={appointment.id} apt={appointment} onStatusChange={onStatusChange} openApptWhatsApp={openApptWhatsApp} handleDeleteAppt={handleDeleteAppt} handleOpenSignature={handleOpenSignature} />
                          ))}
                        </div>
                      </div>
                    );
                  ...
                  return (
                    <div key={time} className="relative flex items-center group cursor-pointer" onClick={() => setShowAddModal(true)}>
                      <div className="absolute -left-10 bg-glass-bg px-1 text-[10px] font-medium text-slate-400 w-8 text-right z-10">{time}</div>
                      ...
                      <div className="min-h-11 flex-1 ml-4 border border-dashed border-slate-200 rounded-[12px] flex items-center px-4 text-[10px] text-slate-400 font-medium group-hover:border-primary/50 group-hover:text-primary transition-colors bg-white/30 backdrop-blur-sm"><Plus size={12} className="mr-1" /> Créneau libre</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-4">
                {dayAppointments.map(apt => (
                  <div key={apt.id} className="relative">
                    <div className="absolute -left-10 mt-3 bg-white px-1 text-[10px] font-bold text-slate-500 w-8 text-right z-10">{normalizeAppointmentTime(apt.time)}</div>
                    <div className="absolute left-[-19px] top-4 w-2.5 h-2.5 rounded-full border-2 border-primary bg-white z-10" />
                    <DraggableApptCard apt={apt} onStatusChange={onStatusChange} openApptWhatsApp={openApptWhatsApp} handleDeleteAppt={handleDeleteAppt} handleOpenSignature={handleOpenSignature} />
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="fixed bottom-32 right-6 z-40">
          <button onClick={() => setShowAddModal(true)} className="w-14 h-14 bg-primary text-white rounded-full shadow-xl flex items-center justify-center" aria-label="Ajouter un rendez-vous"><Plus size={22} /></button>
        </div>
