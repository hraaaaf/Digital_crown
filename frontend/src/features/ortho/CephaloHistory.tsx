...
export const CephaloHistory: React.FC<CephaloHistoryProps> = ({ patientId, onSelect, onDelete }) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/ia/patients/${patientId}/cephalo-analyses`);
        setAnalyses(response.data);
      } catch (error) {
        console.error("Erreur chargement historique céphalométrique:", error);
        toast.error("Impossible de charger l'historique céphalométrique");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (analyses.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-200">
        ...
        <h3 className="text-lg font-black text-slate-400 uppercase tracking-tight">Aucun historique céphalométrique</h3>
        <p className="text-sm text-slate-400 mt-2">Importez une première radiographie céphalométrique.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
        <Activity size={16} /> Historique des Analyses ({analyses.length})
      </h3>
      <div className="grid grid-cols-1 gap-4">
        {analyses.map((analysis) => {
          ...
          const measuresCount = analysis.angles_data?.detections?.length || 0;
          return (
            <div
              key={analysis.id}
            ...
            >
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h4 className="font-black text-slate-800 text-lg tracking-tight">Analyse du {dateStr}</h4>
                  <div className="flex items-center gap-4 mt-1">
                    <span className="flex items-center gap-1.5 text-xs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg">
                      ...
                      {measuresCount} mesure{measuresCount > 1 ? 's' : ''}
                    </span>
                    <span className="flex items-center gap-1.5 text-xs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg">
              ...
              <div className="flex items-center gap-4">
                <button
                  onClick={async (e) => {
                    e.stopPropagation();
                    if (window.confirm('Êtes-vous sûr de vouloir supprimer cette analyse céphalométrique et son image associée ?')) {
                      try {
