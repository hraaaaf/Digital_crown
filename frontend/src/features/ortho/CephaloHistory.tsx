  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      const endpoint = view === 'active'
        ? `/ia/patients/${patientId}/cephalo-analyses`
        : `/ia/patients/${patientId}/cephalo-trash`;
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-200">
          ...
          <h3 className="text-lg font-black text-slate-400 uppercase tracking-tight">
            {view === 'active' ? 'Aucun historique céphalométrique' : 'Corbeille vide'}
          </h3>
          <p className="text-sm text-slate-400 mt-2">
            {view === 'active' ? 'Importez une première radiographie céphalométrique.' : 'Aucune analyse céphalométrique supprimée à restaurer.'}
          </p>
        </div>
      ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-1">
                      <span className="flex items-center gap-1.5 text-xs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg">
                        <Activity size={14} className="text-indigo-500" /> Analyse enregistrée
                      </span>
                      <span className="flex items-center gap-1.5 text-xs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg">
                        <FileText size={14} className="text-emerald-500" /> Données céphalométriques enregistrées
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  {view === 'active' ? (
                    <>
                      <button
                        ...
                        aria-label="Mettre l'analyse céphalométrique à la corbeille"
                      >
                        <Trash2 size={16} />
                      </button>
