...
const SHORT_TITLES: Record<ClinicalStage['stage_id'], string> = {
  R11: 'Diagnostic',
  R12: 'Objectifs',
  R13: 'Options',
  R14: 'Validation',
...
const EvidenceRefs: React.FC<{
  ...
  <div className="min-w-0 rounded-lg border px-2.5 py-2" style={{ borderColor: P.border, background: P.bgInput }}>
    <div className="flex items-start justify-between gap-2">
      ...
      {items.length > 0 && (
        <span className="shrink-0 rounded-full px-1.5 py-0.5 text-[8px] font-black" style={{ background: `${P.accentWarning}14`, color: P.accentWarning }}>
          {items.length}
        </span>
      )}
    </div>
    {items.length ? (
      <ul className="mt-1 space-y-0.5">
        {items.map(item => (
          <li key={item} className="break-words font-mono text-[8px] leading-3.5" style={{ color: P.textMuted }}>{item}</li>
        ))}
      </ul>
    ) : (
...
export const ClinicalScientificStudio: React.FC<Props> = ({ patientId, analysisId, P }) => {
  ...
  return (
    ...
    >
      <div className="border-b px-3 py-3 sm:px-4" style={{ borderColor: P.border }}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-start gap-2.5">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[10px]" style={{ background: `${P.accent}12`, color: P.accent }}>
              <ShieldCheck size={16} />
            </div>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                <h3 className="text-[11px] font-black uppercase tracking-[0.1em]" style={{ color: P.text }}>Chaîne clinique scientifique</h3>
                <span className="rounded-full px-2 py-0.5 text-[8px] font-black tracking-wide" style={{ background: `${P.accent}10`, color: P.accent }}>
                  Diagnostic → décision clinique
                </span>
              </div>
              <p className="mt-1 max-w-3xl text-[10px] leading-4" style={{ color: P.textMuted }}>
          ...
          {snapshot && (
            <div className="flex min-h-8 shrink-0 items-center gap-1.5 rounded-lg border px-2.5 py-1.5" style={{ borderColor: P.border, background: P.bgCard }}>
              <LockKeyhole size={12} style={{ color: snapshot.blocking_gate_count ? P.accentWarning : P.accentSuccess }} />
              <span className="text-[8px] font-black uppercase tracking-wider" style={{ color: P.textMuted }}>
                {snapshot.blocking_gate_count} blocage{snapshot.blocking_gate_count === 1 ? '' : 's'}
              </span>
            </div>
          )}
        </div>
      ...
      {!loading && error && (
        <div className="m-3 flex gap-3 rounded-xl border p-3.5" style={{ borderColor: `${P.accentError}55`, background: `${P.accentError}10` }}>
      ...
      {!loading && snapshot && (
        <div className="p-3 sm:p-4">
            ...
            {snapshot.stages.map(stage => {
              ...
              return (
                <button
                  ...
                  type="button"
                  aria-pressed={active}
                  aria-label={`${stage.stage_id} ${stage.title} — ${stateLabel(stage.presentation_state)}`}
                  onClick={() => setSelectedId(stage.stage_id)}
                  className="min-h-[58px] min-w-0 rounded-[11px] border px-2 py-2 text-left transition-all hover:-translate-y-0.5 sm:min-h-[62px] sm:px-3"
                  style={{
                    background: active ? `${P.accent}0F` : P.bgCard,
                    borderColor: active ? `${P.accent}70` : P.border,
                    boxShadow: active ? `0 0 0 1px ${P.accent}14` : 'none',
                  }}
                >
                  <div className="flex items-center justify-between gap-1">
                    <span className="rounded-md px-1.5 py-0.5 text-[8px] font-black tracking-[0.08em]" style={{ background: `${P.accent}0D`, color: P.textDim }}>
                      {stage.stage_id}
                    </span>
                    <StageStatusIcon stage={stage} color={color} />
                  </div>
                  <div className="mt-1 truncate text-[9px] font-black sm:text-[10px]" style={{ color: P.text }}>{SHORT_TITLES[stage.stage_id]}</div>
                  <div className="mt-0.5 truncate text-[8px] font-bold" style={{ color }}>{stateLabel(stage.presentation_state)}</div>
                </button>
              );
            })}
          </div>
          ...
          {selected && (
            <div className="mt-2.5 overflow-hidden rounded-[16px] border" style={{ borderColor: P.border, background: P.bgCard }}>
              <div className="border-b px-3 py-2.5 sm:px-4" style={{ borderColor: P.border }}>
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  <span className="text-[11px] font-black" style={{ color: P.text }}>{selected.title}</span>
                  <span className="rounded-full border px-2 py-0.5 text-[8px] font-bold uppercase tracking-[0.1em]" style={{ borderColor: P.border, color: P.textDim }}>
                    Réf. technique {selected.stage_id}
                  </span>
                  <span className="rounded-full px-2 py-0.5 text-[8px] font-black" style={{ background: `${statusColor(selected.presentation_state)}12`, color: statusColor(selected.presentation_state) }}>
                    {stateLabel(selected.presentation_state)}
                  </span>
                </div>
                <p className="mt-1 max-w-4xl text-[10px] leading-4" style={{ color: P.textMuted }}>{selected.summary}</p>
              </div>

              <div className="grid gap-2.5 p-2.5 sm:p-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(250px,0.82fr)]">
                <div className="rounded-xl border px-3 py-2.5" style={{ borderColor: P.border, background: P.bgInput }}>
                  <div className="mb-1.5 flex items-center gap-2">
                    <AlertTriangle size={12} style={{ color: P.accentWarning }} />
                    <span className="text-[8px] font-black uppercase tracking-wider" style={{ color: P.text }}>Blocages visibles</span>
                  </div>
                  {selected.blocking_gates.length ? (
                    <ul className="space-y-1">
                      {selected.blocking_gates.map(gate => (
                        <li key={gate} className="text-[9px] leading-3.5" style={{ color: P.textMuted }}>
                  ...
                  ) : <p className="text-[9px]" style={{ color: P.accentSuccess }}>Aucun gate bloquant.</p>}
                </div>

                <div className="rounded-xl border px-3 py-2.5" style={{ borderColor: P.border, background: P.bgInput }}>
                  <div className="mb-1.5 flex items-center gap-2">
                    <Database size={12} style={{ color: P.accent }} />
                    <span className="text-[8px] font-black uppercase tracking-wider" style={{ color: P.text }}>Provenance</span>
                  </div>
                  <dl className="space-y-1">
                    {selected.provenance.map(item => (
                      <div key={`${item.label}-${item.value}`} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2 text-[8.5px] leading-3.5">
                ...
                <aside className="rounded-xl border px-3 py-2.5" style={{ borderColor: `${P.accentWarning}40`, background: `${P.accentWarning}07` }}>
                  <div className="flex items-center gap-2">
                    <LockKeyhole size={12} style={{ color: P.accentWarning }} />
                    <span className="text-[8px] font-black uppercase tracking-wider" style={{ color: P.text }}>Action praticien</span>
                  </div>
                  <p className="mt-1.5 text-[10px] font-black" style={{ color: P.text }}>
                    {selected.clinician_action.available ? selected.clinician_action.label : 'Aucune validation disponible'}
                  </p>
                  <p className="mt-1 text-[9px] leading-4" style={{ color: P.textMuted }}>{selected.clinician_action.reason}</p>
                  <div className="mt-2 border-t pt-2 text-[8px] font-bold leading-3.5" style={{ borderColor: P.border, color: P.textDim }}>
              ...
              <div className="grid gap-1.5 border-t px-2.5 py-2.5 sm:grid-cols-3 sm:px-3" data-testid="r15-scientific-exceptions" style={{ borderColor: P.border }}>
          ...
          <div className="mt-2 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 text-[8px] leading-3.5" style={{ color: P.textDim }}>
