...
describe('M6-I passkey / biometric hardening', () => {
  ...
  it('fails closed on canonical mobile biometric lock responses', () => {
  ...
  it('routes shared mobile clients through UV memory token and relocks after shared 401 normalization', () => {
    const apiSource = source('src/services/api.ts');
    const botSource = source('src/components/CrownBot/CrownBotChat.tsx');
    expect(apiSource).toContain('export function getRuntimeAuthToken');
    expect(apiSource).toContain('MobileStorage.getBiometricAccessToken()');
    expect(apiSource).toContain('status === 423');
    expect(apiSource).toContain('original?._mobileRetried');
    expect(apiSource).toContain('getBiometricVaultEnvelope().catch');
    expect(apiSource).toContain('propagateMobileBiometricLock');
    expect(botSource).toContain("api, API_BASE, getRuntimeAuthToken");
    expect(botSource).toContain('const token = getRuntimeAuthToken();');
  });
