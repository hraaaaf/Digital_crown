...
vi.mock('../services/zka/MobileStorage', () => ({
  MobileStorage: { getCredentials: vi.fn(async () => creds) },
}));
...
beforeEach(() => {
  ...
  mobileFetchMock.mockImplementation((input: RequestInfo | URL, init: RequestInit = {}) => {
    ...
    if (method === 'GET' && url.endsWith('/api/mobile/notifications')) {
      getCount += 1;
      if (getCount === 1) return Promise.resolve(jsonResponse({ total: 2, alerts: initialAlerts }));
      if (getCount === 2) return new Promise<Response>(resolve => { resolveStaleGet = resolve; });
...
describe('M6-D1 notification center', () => {
  it('keeps a read mutation when an older open-triggered GET resolves afterwards', async () => {
  ...
  it('portals the dialog outside its rendering ancestor', async () => {
    const user = userEvent.setup();
