...
const baseUrl = process.env.LOT3_EVIDENCE_URL || 'http://127.0.0.1:5174/superadmin-lot3-audit.html';
const phase = process.env.LOT3_EVIDENCE_PHASE || 'before';
const outputDir = process.env.LOT3_EVIDENCE_DIR || `../artifacts/v1-07-lot3-superadmin-${phase}`;
const viewports = [
...
async function capture(viewport, scenario) {
  ...
  const snapshot = await page.evaluate(() => {
    ...
    const renewal = document.querySelector('[title="WhatsApp de relance"], [title="Email de relance"]');
    return {
      text,
      horizontalOverflow: document.documentElement.scrollWidth > window.innerWidth,
      genericDowngrade: text.includes('Erreur lors du changement de pack.'),
      preciseDowngrade: text.includes('équipe réservée 2 dentiste(s) / 3 assistante(s)'),
      genericRenewal: text.includes("Erreur lors de l'envoi de l'email"),
      preciseRenewal: text.includes("Aucun numéro de téléphone trouvé pour l'envoi WhatsApp."),
      renewalTitle: renewal?.getAttribute('title') || null,
    };
  });
...
try {
  for (const viewport of viewports) {
    for (const scenario of ['downgrade', 'renewal']) {
