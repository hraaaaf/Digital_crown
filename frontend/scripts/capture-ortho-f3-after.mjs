...
const OUTPUT_DIR = path.join(FRONTEND_DIR, 'ortho-f3-after-artifacts');
const PORT = 5197;
...
const htmlSource = `<!doctype html><html lang="fr"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Ortho F3 AFTER</title><style>html,body,#root{width:100%;min-height:100%;margin:0}</style></head><body><div id="root"></div><script type="module" src="/src/ortho-f3-after-entry.tsx"></script></body></html>`;
...
await writeFile(path.join(FRONTEND_DIR,'src','ortho-f3-after-entry.tsx'),entrySource,'utf8');
await writeFile(path.join(FRONTEND_DIR,'ortho-f3-after.html'),htmlSource,'utf8');
...
const report={lot:'V1-05-F3',phase:'AFTER',productHead:PRODUCT_HEAD,viewports:viewports.map(v=>v.name),captures,blockedExternalRequests,invalidCount:invalid.length};
