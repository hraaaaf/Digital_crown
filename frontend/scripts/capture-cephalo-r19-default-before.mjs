...
const SOURCE_PRODUCT_HEAD = '35c4ee606e953f2f2a8a9d91ab540bf6c7ef476a';
const FRONTEND_DIR = process.cwd();
const OUTPUT_DIR = path.join(FRONTEND_DIR, 'cephalo-r19-default-before-artifacts');
const PORT = 5195;
...
`;
const htmlSource = `<!doctype html><html lang="fr"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Cephalo R19 BEFORE default</title><style>html,body,#root{width:100%;height:100%;margin:0}</style></head><body><div id="root"></div><script type="module" src="/src/cephalo-r19-default-before-entry.tsx"></script></body></html>`;
...
await writeFile(path.join(FRONTEND_DIR,'src','cephalo-r19-default-before-entry.tsx'),entrySource,'utf8');
await writeFile(path.join(FRONTEND_DIR,'cephalo-r19-default-before.html'),htmlSource,'utf8');
...
async function captureViewport(viewport,attempt){
 ...
 try{
  const response=await page.goto(`${BASE_URL}/cephalo-r19-default-before.html`,{waitUntil:'domcontentloaded',timeout:30000});
  ...
  for(const mode of modes){
   ...
   await page.screenshot({path:path.join(OUTPUT_DIR,`before-reference-${mode}-${viewport.name}.png`),fullPage:false});
   if(mode==='com'){
     await page.locator('[data-r19-analysis-panel="com"]').scrollIntoViewIfNeeded();
     await page.screenshot({path:path.join(OUTPUT_DIR,`before-com-panel-${viewport.name}.png`),fullPage:false});
   }
...
try{
 await waitForServer(`${BASE_URL}/cephalo-r19-default-before.html`);
...
const invalid=captures.filter(c=>!c.valid); const report={lot:'CEPHALO-R19-ANALYSIS-REFERENCE-LAYOUT',phase:'BEFORE',theme:CAPTURE_THEME,sourceProductHead:SOURCE_PRODUCT_HEAD,productHead:PRODUCT_HEAD,viewports:viewports.map(v=>v.name),modes,capturePolicy:'Pre-feature master product with harness-only default-theme evidence; same deterministic fixture and 390/768/1280 viewports; validates existing R19 analysis layout before semantic-color implementation.',captures,blockedExternalRequests,invalidCount:invalid.length};
