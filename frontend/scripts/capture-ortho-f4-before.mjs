...
const OUTPUT_DIR = path.join(FRONTEND_DIR, 'ortho-f4-before-artifacts');
const PORT = 5198;
...
const htmlSource = `<!doctype html><html lang="fr"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Ortho F4 BEFORE</title><style>html,body,#root{width:100%;min-height:100%;margin:0}</style></head><body><div id="root"></div><script type="module" src="/src/ortho-f4-before-entry.tsx"></script></body></html>`;
...
await writeFile(path.join(FRONTEND_DIR,'src','ortho-f4-before-entry.tsx'),entrySource,'utf8');
await writeFile(path.join(FRONTEND_DIR,'ortho-f4-before.html'),htmlSource,'utf8');
...
for(const viewport of viewports){
  ...
  try{
    await waitForServer(`${BASE_URL}/ortho-f4-before.html`);
    const response=await page.goto(`${BASE_URL}/ortho-f4-before.html`,{waitUntil:'domcontentloaded',timeout:30000});
    ...
    const metrics=await page.evaluate(()=>{
      ...
      return {
        ...
        hasF3:Boolean(document.querySelector('[data-ortho-f3-compare]')),
        hasJourney:Boolean(document.querySelector('[aria-label="Prochaine action factuelle"]')),
        hasF4:Boolean(document.querySelector('[data-ortho-f4-cockpit]')) || text.includes('cockpit orthodontique'),
      };
    });
    const valid=response?.status()===200&&!metrics.horizontalOverflow&&metrics.hasF3&&metrics.hasJourney&&!metrics.hasF4&&consoleErrors.length===0&&pageErrors.length===0;
...
const report={lot:'V1-05-F4',phase:'BEFORE',productHead:PRODUCT_HEAD,viewports:viewports.map(v=>v.name),captures,blockedExternalRequests,invalidCount:invalid.length};
