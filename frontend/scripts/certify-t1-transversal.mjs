...
if (!patientsResponse.ok()) throw new Error(`Patients fetch failed: ${patientsResponse.status()} ${await patientsResponse.text()}`);
...
async function closeLivePreview(targetPage) {
  const preview = targetPage.getByRole('dialog').filter({ has: targetPage.locator('#document-studio-live-preview-title') }).last();
...
const pathFor = (patientId, documentTab = 'libre') => `/patients/${patientId}?tab=admin&documentTab=${documentTab}`;
...
async function waitForDocumentTab(targetPage, slug) {
  await targetPage.waitForFunction((expected) => new URLSearchParams(window.location.search).get('documentTab') === expected, slug, { timeout: 15000 });
}
...
try { await page.goto(`http://127.0.0.1:5173${bPath}`, { waitUntil: 'domcontentloaded', timeout: 90000 }); }
catch (error) { if (!String(error).includes('net::ERR_ABORTED')) throw error; }
...
await page.route(delayedPattern, async (route) => {
  ...
  try {
    ...
    if (firstDelayedAResolve) { firstDelayedAResolve(); firstDelayedAResolve = null; }
    await releaseDelayedAPromise;
    await route.fulfill({ response });
    delayedAFulfilled += 1;
  } catch (error) { delayedARouteErrors.push(String(error)); }
});
await spaNavigate(page, pathFor(patientA.id));
await Promise.race([firstDelayedA, new Promise((_, reject) => setTimeout(() => reject(new Error('No delayed patient A request observed')), 15000))]);
await spaNavigate(page, bPath);
await page.getByRole('heading', { name: patientBName, exact: true }).waitFor({ timeout: 30000 });
const bAuthoritativeBeforeRelease = page.url().includes(`/patients/${patientB.id}`) && await page.getByRole('heading', { name: patientBName, exact: true }).isVisible();
releaseDelayedA();
...
try { await confirmPage.goto(`http://127.0.0.1:5173${bPath}`, { waitUntil: 'domcontentloaded', timeout: 90000 }); }
catch (error) { if (!String(error).includes('net::ERR_ABORTED')) throw error; }
...
await closeLivePreview(confirmPage);
...
const evidence = {
  patientBoundary: {
    patientA: { id: patientA.id, dossier: patientA.numero_dossier }, patientB: { id: patientB.id, dossier: patientB.numero_dossier },
    delayedARequests, delayedAFulfilled, delayedARouteErrors, bAuthoritativeBeforeRelease, bAuthoritativeAfterRelease,
    pass: delayedARequests > 0 && delayedAFulfilled > 0 && delayedARouteErrors.length === 0 && bAuthoritativeBeforeRelease && bAuthoritativeAfterRelease,
  },
  urlDirtyBoundary: {
    cancelRestoredUrl, cancelPreservedDraft, confirmDirtyArmed, confirmReachedTarget, isolatedConfirmationSession: true,
    pass: cancelRestoredUrl && cancelPreservedDraft && confirmDirtyArmed && confirmReachedTarget,
  },
  clinicalBoundary: { companionAbsent, aiDiagnosticRequests, aiDiagnosticAbsent, pass: companionAbsent && aiDiagnosticAbsent },
  pageErrors,
};
evidence.status = evidence.patientBoundary.pass && evidence.urlDirtyBoundary.pass && evidence.clinicalBoundary.pass && pageErrors.length === 0 ? 'PASS' : 'FAIL';
