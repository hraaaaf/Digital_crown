...
const baseUrl = process.env.PC03_BEFORE_URL || 'http://127.0.0.1:5176';
const outputRoot = process.env.PC03_EVIDENCE_DIR || '../artifacts/pc03-questionnaires-before';
...
async function installHarness(context) {
  await context.addInitScript(({ token }) => {
    ...
    window.fetch = async (input, init = {}) => {
      ...
      if (url.pathname === '/api/patient-companion/pair') {
        return new Response(JSON.stringify({
          access_token: token,
          context: { access_id: 'pc03-before-access', relationship_type: 'SELF', patient: { display_name: 'Aya Audit' } },
          paired_at: '2026-09-20T18:00:00Z',
        }), { status: 201, headers: { 'Content-Type': 'application/json' } });
      }
      if (url.pathname.endsWith('/agenda') || url.pathname.endsWith('/shares')) {
...
for (const [browserName, browserType] of Object.entries(browsers)) {
  ...
  try {
    for (const viewport of viewports) {
      ...
      if (await page.locator('[data-pc03-questionnaires]').count()) throw new Error('PC03 unexpectedly present in BEFORE');
      const horizontalOverflow = await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth);
      if (horizontalOverflow) throw new Error('BEFORE horizontal overflow');
