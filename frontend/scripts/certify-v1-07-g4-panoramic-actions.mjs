...
for(const viewport of [{width:390,height:844},{width:1280,height:900}]){
 ...
 const editInputs=page.locator('input').filter({visible:true}); const marker=`### G4 PANO EDIT ${viewport.width}`;
 const candidate=page.locator('input').filter({hasNot:page.locator('[type=range]')});
