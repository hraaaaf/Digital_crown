...
const invalid = captures.filter(item =>
  ...
  item.metrics.horizontalDocumentOverflow ||
  item.metrics.assistantClipped ||
  item.metrics.headerClipped ||
  !item.metrics.hasCandidateLabel ||
  !item.metrics.hasAutoAction ||
  !item.metrics.hasManualFallback ||
  !item.metrics.hasWorkspaceHeading
);
