...
const outDir = path.resolve('../artifacts/patient-p0d-output-after');
...
async function snap(page, protocol, phase, viewport, pageErrors, httpErrors) {
  const focus = phase === 'result'
    ...
    : page.locator('text=/Synthèse des observations endodontiques|Synthèse des observations et signaux de vigilance/').first();
  ...
  const screenshot = `patient-p0d-${protocol}-${phase}-after-${viewport.width}x${viewport.height}.png`;
  await page.screenshot({ path: path.join(outDir, screenshot), fullPage: false });
  evidence.push({ protocol, phase, viewport, screenshot, focusedText, metrics, pageErrors: [...pageErrors], httpErrors: [...httpErrors] });
}
...
const banned = /Pulpite Irréversible|Pulpite Réversible|Nécrose pulpaire|Amoxicilline|Métronidazole|Antibiothérapie|Pulpectomie|Traitement Canalaire|Drainage chirurgical|Radiographie RVG|hospitalière/i;
const summary = {
  ...
  relevantHttpErrors,
  safeOutputEvidence: resultEvidence.map(e => ({
    protocol: e.protocol,
    viewport: e.viewport,
    hasObservations: e.focusedText.includes('Observations recueillies'),
    hasPractitionerDecision: e.focusedText.includes('Diagnostic et conduite thérapeutique') && e.focusedText.includes('décision du praticien'),
    hasVigilanceWhenUrgent: e.protocol !== 'urgence' || e.focusedText.includes('Vigilance'),
...
if (summary.safeOutputEvidence.some(e => !e.hasObservations || !e.hasPractitionerDecision || !e.hasVigilanceWhenUrgent || e.hasBannedOutput)) {
  console.error('Fail-closed output contract failed.');
  console.error(JSON.stringify(summary.safeOutputEvidence, null, 2));
  process.exit(1);
}
await browser.close();
await api.dispose();
