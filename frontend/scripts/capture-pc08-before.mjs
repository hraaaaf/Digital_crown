...
const baseUrl = process.env.PC08_BEFORE_URL || 'http://127.0.0.1:5182';
const outputRoot = process.env.PC08_EVIDENCE_DIR || '../artifacts/pc08-emergency-photo-before';
      ...
      if (url.pathname === '/api/patient-companion/pair' && method === 'POST') {
        return new Response(JSON.stringify({
          ...
          context: {
            ...
            patient: { display_name: 'Aya Companion' },
          },
...
for (const [browserName, browserType] of Object.entries(browsers)) {
  ...
  try {
    for (const viewport of viewports) {
      ...
      if (await page.locator('[data-pc08-secure-messaging]').count()) throw new Error('PC08 unexpectedly present in BEFORE');
      const horizontalOverflow = await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth);
      if (horizontalOverflow) throw new Error('PC08 BEFORE horizontal overflow');
