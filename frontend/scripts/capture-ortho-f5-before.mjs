...
const OUTPUT_DIR = path.join(FRONTEND_DIR, 'ortho-f5-before-artifacts');
const PORT = 5196;
...
const htmlSource = `<!doctype html><html lang="fr"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Ortho F5 BEFORE</title><style>html,body,#root{width:100%;min-height:100%;margin:0}</style></head><body><div id="root"></div><script type="module" src="/src/ortho-f5-before-entry.tsx"></script></body></html>`;
...
await writeFile(path.join(FRONTEND_DIR,'src','ortho-f5-before-entry.tsx'),entrySource,'utf8');
await writeFile(path.join(FRONTEND_DIR,'ortho-f5-before.html'),htmlSource,'utf8');
...
for(const viewport of viewports){
  ...
  try{
    await waitForServer(`${BASE_URL}/ortho-f5-before.html`);
    const response=await page.goto(`${BASE_URL}/ortho-f5-before.html`,{waitUntil:'domcontentloaded',timeout:30000});
    ...
    const metrics=await page.evaluate(()=>{
      ...
      return {
        ...
        forbidden:['amélior','aggrav','succès','echec','échec','worsen','improv','success','failure'].filter(k=>text.includes(k)),
      };
    });
    const valid=response?.status()===200&&!metrics.horizontalOverflow&&metrics.hasCompareSurface&&metrics.hasT0&&metrics.hasT1&&metrics.hasEvidence&&metrics.hasNumericCaption&&metrics.hasDelta&&metrics.forbidden.length===0&&consoleErrors.length===0&&pageErrors.length===0;
    await page.screenshot({path:path.join(OUTPUT_DIR,`before-overview-${viewport.name}.png`),fullPage:false});
...
const report={lot:'V1-05-F5',phase:'BEFORE',productHead:PRODUCT_HEAD,baselineCommit:process.env.F5_BEFORE_BASE||PRODUCT_HEAD,viewports:viewports.map(v=>v.name),captures,blockedExternalRequests,invalidCount:invalid.length};
