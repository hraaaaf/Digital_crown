...
const baseUrl = process.env.PC04_BEFORE_URL || 'http://127.0.0.1:5178';
const outputRoot = process.env.PC04_EVIDENCE_DIR || '../artifacts/pc04-consent-before';
const viewports = [{ width: 360, height: 800 }, { width: 390, height: 844 }];
const browsers = { chromium, webkit };
const evidence = [];
const token = 'eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJwYzA0LWJhc2UiLCJleHAiOjIwMDAwMDAwMDB9.audit';
    ...
    window.fetch = async (input, init = {}) => {
      ...
      if (url.pathname === '/api/patient-companion/pair' && method === 'POST') {
        return new Response(JSON.stringify({
          access_token: token,
          context: { access_id: 'pc04-before-access', relationship_type: 'SELF', patient: { display_name: 'Aya Audit' } },
...
for (const [browserName, browserType] of Object.entries(browsers)) {
  ...
  try {
    for (const viewport of viewports) {
      ...
      if (await page.locator('[data-pc04-consent-vault]').count()) throw new Error('PC04 unexpectedly present in BEFORE');
      const horizontalOverflow = await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth);
      if (horizontalOverflow) throw new Error('PC04 BEFORE horizontal overflow');
